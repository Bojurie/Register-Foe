...
const ElectionDisplayFetcher = () => {
  ...
  const daysSinceEnd = (currentDate - endDate) / (1000 * 60 * 60 * 24);

  // Determine if we should display the winning candidate and vote percentage
  const showResults = currentDate < endDate || daysSinceEnd <= 2;
  ...
  return (
    <ElectionDisplay
      ...
      leadingCandidate={
        showResults
          ? `${leadingCandidateDetails.firstName} ${leadingCandidateDetails.lastName}`
          : "Election ended - results no longer displayed"
      }
      votePercentage={showResults ? votePercentage : "N/A"}
