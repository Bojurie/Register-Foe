...
const CityScene = () => {
  ...
  useEffect(() => {
    const city = cityRef.current || new THREE.Group(); // Initialize city as a new THREE.Group if not present
    ...
    try {
      ...
      if (camera) {
        ...
        console.error("Camera not found in the city.");
      }
    } catch (error) {
      console.error("Error in CityScene setup:", error);
    }

    // Set cityRef to the updated city object
