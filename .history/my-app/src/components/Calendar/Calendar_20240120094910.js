...
const Calendar = () => {
  ...
  const [reminders, setReminders] = useState([]); // Initialize reminders state
  ...
  useEffect(() => {
    // Fetch user reminders and update datesWithReminders on login
...
const handleDayClick = (day) => {
  ...
  // Check if the clicked date has a reminder
  const hasReminder = datesWithReminders.some(
    (date) => date.toDateString() === day.toDateString()
  );

  if (hasReminder) {
    // If there is a reminder, open the modal to view it
    setFullModalIsOpen(true);
    let remindersForDay = reminders.filter(
      (reminder) => reminder.date === day.toDateString()
    );
    setSelectedReminder(remindersForDay[0]);
  } else {
    // If no reminder, open the modal to enter and save a new reminder
    setNewReminderModalIsOpen(true); // Use the correct state variable here
  }
};
