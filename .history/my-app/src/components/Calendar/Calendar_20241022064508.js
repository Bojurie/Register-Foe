...
const Calendar = () => {
  ...
  const [noReminders, setNoReminders] = useState(false); // State for no reminders
  const [totalUpcomingElections, setTotalUpcomingElections] = useState(0); // State for upcoming elections

  // Fetch reminders and upcoming elections for the logged-in user
  const fetchUserReminders = useCallback(async () => {
    ...
    try {
      ...
      if (response.activeReminders.length === 0) {
        setNoReminders(true);
      } else {
        setReminders(response.activeReminders);
        setNoReminders(false);
      }

      setTotalUpcomingElections(response.totalUpcomingElections);
    } catch (error) {
      setNoReminders(true);
      console.error("Error fetching reminders or elections:", error);
      toast.error(
        "Failed to load reminders or elections. Please try again later."
      );
    }
  ...
  return (
    <motion.div className="calendar-wrapper">
      <div className="calendar-header">
        ...
        <p>
          Upcoming Reminders: {reminders.length} | Upcoming Elections:{" "}
