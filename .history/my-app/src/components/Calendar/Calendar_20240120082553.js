...
const getSavedReminder = async () => {
  try {
    const remindersResponse = await getUserReminder();
    console.log("User Reminder Data:", remindersResponse);

    const savedReminder = remindersResponse.find(
      (reminder) => reminder.date === selectedDate.toDateString()
    );

    setSelectedReminder(savedReminder);
    setDatesWithReminders(
      remindersResponse.map((reminder) => new Date(reminder.date))
    );
  } catch (error) {
