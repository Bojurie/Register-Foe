...
const Calendar = () => {
  ...
  const [noReminders, setNoReminders] = useState(false); // New state to handle no reminders
  ...
  const fetchUserReminders = useCallback(async () => {
    ...
    try {
      ...
      if (response.length === 0) {
        setNoReminders(true); // Set flag if no reminders found
      } else {
        setReminders(response);
        setNoReminders(false); // Clear flag if reminders are found
      }
    } catch (error) {
      if (error.response?.status === 404) {
        setNoReminders(true);
        console.warn("No reminders found for this user."); // Gracefully handle 404 error
      } else {
    ...
    <motion.div className="calendar-wrapper">
      ...
      {noReminders && (
        <motion.p className="no-reminders-text">
          No reminders available for this user.
        </motion.p>
      )}
