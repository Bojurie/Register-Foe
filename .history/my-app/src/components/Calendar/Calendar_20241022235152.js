  ...
  const fetchUserReminders = useCallback(async () => {
    ...
    try {
      ...
      if (response.activeReminders.length === 0) {
        ...
        const fetchedReminders = response.activeReminders.map((reminder) => ({
          ...reminder,
          date: new Date(reminder.date), // Ensure the date is in Date object format
        }));
        setReminders(fetchedReminders); // Store reminders in state
