...
const handleDayClick = async (day) => {
  const clickedDateReminders = reminders.filter(
  ...
  if (clickedDateReminders.length > 0) {
    ...
    if (reminderText) {
      ...
      try {
        ...
        // Fetch reminders for the selected date using getUserReminder
        const updatedReminders = await fetchRemindersForDate(reminderDate);
...
const fetchRemindersForDate = async (date) => {
  try {
    const response = await getUserReminder("/api/reminders", {
      params: {
        userId: user?.id,
      ...
      <AnimatePresence>
        {selectedReminder && (
          <div className="reminder">
            ...
            <button onClick={() => scheduleNotification(selectedReminder)}>
