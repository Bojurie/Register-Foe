...
const handleDayClick = async (day) => {
  ...
  try {
    const response = await getUserReminder(reminderDate);
    const clickedDateReminders = response.data;

    if (clickedDateReminders.length > 0) {
      // If a reminder already exists, display the existing reminder
      setReminders(clickedDateReminders);
      setSelectedReminder(clickedDateReminders[0]);
    } else {
      // If no reminder exists, set a new reminder
      setNewReminderText("");
      setSelectedReminder(null); // Set selectedReminder to null for new reminders
    }
