  ...
  const handlePrevNextMonth = (increment) => {
   ...
   const handleDayClick = (day) => {
    ...
    if (reminderText) {
      ...
      const newReminders = [...reminders, { date: reminderDate, text: reminderText }];
      setReminders(newReminders);
      setSelectedReminder({ date: reminderDate, text: reminderText });
      setHasIndicator(true);
      setSelectedDate(day);
    } else {
      const existingReminder = reminders.find((reminder) => reminder.date.toDateString() === day.toDateString());
      if (existingReminder) {
