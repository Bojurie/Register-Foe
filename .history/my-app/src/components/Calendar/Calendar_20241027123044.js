...
const Calendar = () => {
  ...
  // Function to fetch user's timezone based on geolocation
  ...
  // Accurate "today" based on user's timezone
  ...
  const fetchUserReminders = useCallback(async () => {
    ...
    try {
      ...
      if (response?.length) {
        setReminders(response.map(reminder => ({ ...reminder, date: new Date(reminder.date) })));
  ...
  const handleDayClick = (day) => {
    ...
    setSelectedDate(day);
    const reminder = reminders.find(reminder => reminder.date.toDateString() === day.toDateString());
  ...
  return (
    <motion.div className="calendar-wrapper">
      ...
      <motion.div className="calendar-days">
        ...
        {monthDays.map(day => {
          const hasReminder = reminders.some(reminder => reminder.date.toDateString() === day.toDateString());
          return (
            <motion.div
              key={day.toString()}
              className={`calendar-day ${hasReminder ? "with-reminder" : ""} ${day.toDateString() === today.toDateString() ? "today" : ""}`}
      ...
      {noReminders && <motion.p className="no-reminders-text">No reminders found for this user.</motion.p>}
