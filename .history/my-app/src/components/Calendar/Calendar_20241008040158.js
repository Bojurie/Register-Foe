...
const Calendar = () => {
  ...
  const handleSaveReminder = async () => {
    console.log("Save Reminder button clicked"); // Log when the button is clicked

    if (!newReminderText.trim()) {
      toast.warning("Reminder text cannot be empty.");
      console.log("Reminder text is empty. Cannot save."); // Log if the reminder text is empty
      return;
    }

    if (!user?.id) {
      toast.error("User ID is missing. Please log in again.");
      console.log("User ID is missing. Cannot save reminder."); // Log if the user ID is missing
      return;
    }
    ...
    try {
      ...
      console.log("Reminder saved successfully!"); // Log success

      closeModal(); // Close the modal after saving
      fetchUserReminders(); // Refresh the list of reminders
      toast.success("Reminder added successfully!");
    } catch (error) {
      console.error("Error saving reminder:", error); // Log any error that occurs
      toast.error(
