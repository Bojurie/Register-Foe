...
const Calendar = () => {
  ...
  const { postUserReminder } = useAuth();
  ...
  const handleDayClick = async (day) => {
    ...
    if (clickedDateReminders.length > 0) {
      ...
      setModalIsOpen(true); // Open the modal for viewing the reminder
    } else {
      ...
      if (reminderText) {
        ...
        try {
          // Replace 'user123' with the actual user ID from your authentication system
