...
import { FaBell } from "react-icons/fa"; // Icon for new news
import "./NewsItem.css";
...
const NewsItem = ({ newsItem, onClick }) => {
  ...
  return (
    <motion.div
      className={`news-item-container ${newsItem.isNew ? "news-item-new" : ""}`} // Highlight new news
      whileHover={{ scale: 1.02 }}
      onClick={onClick}
    >
      {newsItem.isNew && (
        <div className="news-item-new-indicator">
          <FaBell className="new-news-icon" /> New
        </div>
      )}
      {newsItem.image && (
