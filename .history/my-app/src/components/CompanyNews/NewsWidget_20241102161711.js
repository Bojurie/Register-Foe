...
import { NewsWidgetWrapper } from "../StyledComponents";
...
const NewsWidget = ({ news }) => {
  ...
  const handleNewsClick = async (newsItem) => {
    ...
    if (newsItem.isNew) {
      const { success, message: responseMessage } = await markAsReadNews(
        newsItem._id
      );
      if (success) {
        ...
        setMessage({
          type: "warning",
          text: responseMessage || "Failed to mark as read.",
        });
      }
    }
  };
  ...
  return (
    <NewsWidgetWrapper className="NewsWidget-Wrapper">
