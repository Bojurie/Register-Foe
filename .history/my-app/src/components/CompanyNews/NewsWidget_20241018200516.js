...
const NewsWidget = ({ news: initialNews = [] }) => {
  ...
  const handleNewsClick = async (newsItem) => {
    ...
    if (newsItem.isNew) {
      try {
        ...
        setLoading(true);
        const { data } = await axiosInstance.get(`/news/byCompanyCode/${user.companyCode}`);
  ...
  return (
    <motion.div className="NewsWidget-Wrapper">
      <h2 onClick={() => setIsCollapsed(!isCollapsed)} className="NewsWidget-Header">
              ...
              {loading ? (
                ...
                updatedNews.map((newsItem) => (
                  <motion.div key={newsItem._id}>
                    <NewsItem newsItem={newsItem} onClick={() => handleNewsClick(newsItem)} />
                  </motion.div>
                ))
              ) : (
      ...
      {isModalOpen && (
        <NewsModal newsItem={selectedNewsItem} onClose={handleCloseModal} isFullScreen />
      )}
    </motion.div>
  );
};
