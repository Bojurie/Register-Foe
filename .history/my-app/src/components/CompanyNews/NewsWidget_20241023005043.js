...
const handleNewsClick = async (newsItem) => {
  ...
  if (newsItem.isNew) {
    try {
      ...
      // Update the news item's status in the UI (remove 'isNew' status)
      const updatedNewsList = updatedNews.map((item) => {
        if (item._id === newsItem._id) {
          return { ...item, isNew: false }; // Update isNew to false
        }
        return item;
      });
      ...
      // Fetch updated news from the server if needed
    ...
    console.log(`News item ${newsItem._id} has already been marked as read.`);
  }
};
