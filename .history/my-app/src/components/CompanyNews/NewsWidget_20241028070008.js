...
const NewsWidget = ({ news }) => {
  const { user, markAsReadNews, GetUpComingNews } = useAuth();

  // Define state variables for loading, updated news, and messages
  const [selectedNewsItem, setSelectedNewsItem] = useState(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isCollapsed, setIsCollapsed] = useState(true);
  const [updatedNews, setUpdatedNews] = useState([]); // Define setUpdatedNews here
  const [loading, setLoading] = useState(false); // Define setLoading here
  const [message, setMessage] = useState({ type: "", text: "" });

  // Set the initial news passed in from props to updatedNews state
  useEffect(() => {
