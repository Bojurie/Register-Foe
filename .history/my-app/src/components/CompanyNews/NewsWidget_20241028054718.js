...
const NewsWidget = ({ news }) => {
  ...
  const fetchLatestNews = async () => {
    try {
      ...
      console.log(
        "[INFO] Fetched latest news data:",
        updatedNewsResponse.data.news
      );
    } catch (error) {
  ...
  const handleNewsClick = async (newsItem) => {
    ...
    if (newsItem.isNew) {
      try {
        ...
        if (response?.success) {
          // Update UI state for immediate visual feedback
          ...
          await fetchLatestNews(); // Ensure latest status is fetched
        } else {
          setMessage({
            type: "warning",
