...
const NewsWidget = ({ news: { news = [] } = {} }) => {
  ...
  const [updatedNews, setUpdatedNews] = useState(news || []); // Fallback to an empty array if undefined
  const [loading, setLoading] = useState(false);

  // Ensure updated news from props is reflected in state
  useEffect(() => {
    setUpdatedNews(news || []); // Fallback to an empty array if undefined
  }, [news]);
  ...
  const handleNewsClick = async (newsItem) => {
    ...
    if (newsItem.isNew) {
      try {
        ...
        setUpdatedNews(updatedNewsResponse.data.news || []); // Fallback to an empty array if undefined
      } catch (error) {
