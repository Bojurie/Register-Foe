...
const NewsWidget = ({ news = [] }) => {
  console.log("News passed to NewsWidget:", news); // Debugging: log news data

  const { enqueueSnackbar, updateReadNewNews, user } = useAuth();
  const [selectedNewsItem, setSelectedNewsItem] = useState(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isCollapsed, setIsCollapsed] = useState(false);
  const [updatedNews, setUpdatedNews] = useState(news); // Local state to handle updated news
  ...
  const fetchUpdatedNews = async () => {
    try {
      ...
      console.log("Updated news fetched:", updatedNewsResponse.data); // Debugging: log fetched news
  ...
  // Update local state when the incoming news prop changes
  useEffect(() => {
    console.log("News prop updated:", news); // Debugging: log updated news prop
    setUpdatedNews(news);
  }, [news]);
