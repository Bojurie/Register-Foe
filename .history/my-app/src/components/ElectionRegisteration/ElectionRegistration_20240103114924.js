...
function ElectionRegistrationForm() {
  ...
  useEffect(() => {
    // Fetch candidates and set company code if user and company code are available
    const fetchData = async () => {
      if (user && user.companyCode) {
        try {
          const response = await fetchUserByCompanyCode(user.companyCode);
          setCandidatesList(response || []);
          setCompanyCode(user.companyCode); // Set the company code
        } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await handleCreateElection({
        ...
        companyCode: companyCode, // Use the companyCode from the state
      });
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-3">
          ...
          <input
            ...
            id="companyCode"
            name="companyCode"
            value={companyCode} // Use companyCode from the state
            readOnly
        ...
        <div className="mb-3">
          ...
          <select multiple onChange={handleCandidateSelection}>
