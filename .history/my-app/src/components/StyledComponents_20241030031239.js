...
export const ThemeProvider = ({ children }) => {
  ...
  const updateTheme = (weatherIcon, hour) => {
    if (weatherIcon.includes("01d")) {
      ...
      weatherIcon.includes("02d") ||
      weatherIcon.includes("03d") ||
      weatherIcon.includes("04d")
    ) {
      setCurrentTheme(themes.cloudyDay);
    } else if (
      weatherIcon.includes("02n") ||
      weatherIcon.includes("03n") ||
      weatherIcon.includes("04n")
    ) {
      setCurrentTheme(themes.cloudyNight);
    } else if (weatherIcon.includes("09d") || weatherIcon.includes("10d")) {
      setCurrentTheme(themes.rainyDay);
    } else if (weatherIcon.includes("09n") || weatherIcon.includes("10n")) {
      setCurrentTheme(themes.rainyNight);
    } else if (weatherIcon.includes("13d") || weatherIcon.includes("13n")) {
      setCurrentTheme(themes.snowyDay);
    } else if (weatherIcon.includes("11d") || weatherIcon.includes("11n")) {
