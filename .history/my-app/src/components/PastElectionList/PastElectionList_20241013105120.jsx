    ...
    const fetchPastElections = async () => {
      ...
      try {
        const response = await axiosInstance.get(
          `/pastElection/voted/${userId}`
        );
        const fetchedVotedPastElections =
          response.data.votedPastElections || []; // Ensure data is an array
        setPastElections(fetchedVotedPastElections);
      } catch (err) {
        setError(err);
        setPastElections([]); // Set as empty array in case of error
      } finally {
        setLoading(false);
      }
    };
