...
const ElectionDisplayFetcher = () => {
  ...
  // If candidates are populated, you can find the leading candidate's details here
  const candidates = currentElection.candidates || [];
  const leadingCandidateDetails = candidates.length > 0 ? candidates[0] : {};

  const leadingFirstName =
    leadingCandidateDetails.firstName || "Unknown First Name";
  const leadingLastName =
    leadingCandidateDetails.lastName || "Unknown Last Name";
