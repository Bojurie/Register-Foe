...
const refreshTokenFunction = async () => {
  try {
    const refreshToken = localStorage.getItem("refreshToken"); // Or your secure storage solution
    if (!refreshToken) {
    ...
    // Consider using a more secure way to handle the access token
...
const login = async (formData) => {
  try {
    ...
    return response.data; // Return the data object directly for simplicity
  } catch (error) {
    console.error("Login error:", error.response?.data?.error || error.message);

    // Rethrow a simplified error object
    throw {
