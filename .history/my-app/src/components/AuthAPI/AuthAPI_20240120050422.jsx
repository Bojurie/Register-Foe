...
const getUserById = async (userId) => {
  try {
    ...
    const response = await axiosInstance.get(`/user/users/byId/${idString}`);
...
const newReminder = async (formData) => {
  try {
    ...
    console.error("Error in newReminder:", error.message);
    throw error;
  }
};

const fetchReminder = async (formData) => {
  try {
    const response = await axiosInstance.get("/reminder/reminders", formData);
    return response.data;
  } catch (error) {
    console.error("Error in fetchReminder:", error.message);
    throw error;
  }
};
...
const getElectionsByTheirId = async (companyCode, electionId) => {
  if (process.env.NODE_ENV === "development") {
    console.log(
      `Received companyCode: ${companyCode}, electionId: ${electionId}`
    );
  }
