...
const getAdminUsers = async (companyCode) => {
  try {
    const response = await axiosInstance.get(
      `/user/admin/byCompanyCode/${companyCode}`
    );
    return response.data;
  } catch (error) {
    const errorMessage =
...
const saveElection = async (electionData) => {
  try {
    ...
    if (response.status === 201) {
      // Check if response data and success property exist
    ...
    return { success: false, error: "Failed to save election" };
  } catch (error) {
    console.error("Error saving election:", error);
    enqueueSnackbar("Failed to save election", { variant: "error" });
    return {
      success: false,
      error: "Failed to save election due to a network error",
    };
  }
};
