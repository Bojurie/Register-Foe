...
const GetUpComingElections = async (companyCode) => {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      return { data: elections || [] }; // Ensure `data` is an array, even if empty
    } else {
...
const createTopic = async (formData) => {
  try {
    const response = await axiosInstance.post("/vote/vote/topics", formData); // Adjust the endpoint for creating a topic
    return response.data;
  } catch (error) {
...
const GetUpComingNews = async (companyCode) => {
  ...
  if (!companyCode) {
    console.warn("[WARN] No company code provided to GetUpComingNews.");
  ...
  try {
    ...
    const errorMessage =
      error.response?.data?.message || "Failed to fetch updated news.";
    console.error(
      "[ERROR] Error fetching news in GetUpComingNews:",
      errorMessage
    );
    return { news: [], message: errorMessage };
  }
};
...
const MarkNewsAsReading = async (newsId) => {
  try {
    const response = await axiosInstance.patch(`/news/markAsRead/${newsId}`);
    if (response.data?.success) {
      console.log(`[INFO] Marked news ${newsId} as read successfully.`);
      return response.data;
    } else {
    ...
    const errorMessage =
      error.response?.data?.message || "Error marking news as read.";
    console.error("[ERROR] Error marking news as read:", errorMessage);
    throw new Error(errorMessage);
  }
};
...
const getReactionsNow = async (topicId, action) => {
  try {
    ...
    if (
      response.data &&
      (response.data.likesCount !== undefined ||
        response.data.dislikesCount !== undefined)
    ) {
      return response.data;
    } else {
      throw new Error(
        `Invalid response for ${action}: likesCount or dislikesCount is missing.`
      );
    }
  } catch (error) {
...
const apiUpdateTopicVote = async (userId, topicId, action) => {
  try {
    const response = await axiosInstance.post(
      `/vote/vote/topics/${topicId}/${action}`,
      { userId }
    );
    return response.data;
  } catch (error) {
    console.error(`Error updating vote (${action}):`, error.message);
    throw new Error("Could not update vote. Please try again.");
  }
};
