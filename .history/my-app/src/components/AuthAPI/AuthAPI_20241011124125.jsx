...
try {
  enqueueSnackbar(`Successfully fetched ${action}.`, { variant: "success" });
  return response.data;
} catch (error) {
  enqueueSnackbar(`Failed to fetch ${action}. Please try again.`, {
    variant: "error",
  });
...
const getReactionsNow = async (topicId, action) => {
  ...
  const response = await axiosInstance.get(`/vote/topics/${topicId}/${action}`);
    ...
    if (!response.data || response.status !== 200) {
      throw new Error(
        `Failed to fetch ${action}. Server responded with: ${response.statusText}`
      );
    }
