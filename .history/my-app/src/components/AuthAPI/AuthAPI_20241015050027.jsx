...
const GetUpComingElections = async (companyCode) => {
  ...
  try {
    const response = await axiosInstance.get(`/election/elections/${companyCode}`);

    if (response.status === 200) {
      const { elections, message } = response.data;
      ...
      if (elections && elections.length > 0) {
        ...
        console.warn("No upcoming elections found:", message);
        return []; // Return an empty array if no elections
      }
    }
  } catch (error) {
...
const getPastElections = async (companyCode) => {
  ...
  try {
    ...
    // Handle 404 error specifically and return an empty array
    if (error.response && error.response.status === 404) {
      console.warn(`No past elections found for company code: ${companyCode}`);
      return []; // Gracefully handle by returning an empty array
    }
...
const GetUpComingNews = async (companyCode) => {
  ...
  try {
    ...
    console.log("News data fetched:", response.data); // Log the response data here
    return response.data; // Assuming the API returns { news: [...] }
  } catch (error) {
    handleApiError(error, "Failed to fetch upcoming news");
    return { news: [], message: "Failed to fetch news." }; // Fallback response in case of failure
  }
};
...
const sendMessage = async (messageData) => {
  try {
    ...
    const response = await axiosInstance.post(`/message/messages`, messageData); // Ensure correct route is used
