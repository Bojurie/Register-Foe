...
const createElection = async (formData) => {
  try {
    ...
    while (!isUnique) {
      electionId = uuidRandom();
      formData.electionId = electionId;

      const existingElection = await axiosInstance.get(
        `/election/check-unique/${formData.companyCode}/${electionId}`
      );
      isUnique = !existingElection.data.exists;
    }
