...
const getConversations = async () => {
  try {
    ...
    if (response.status !== 200) {
      ...
      return []; // Return empty array if response isn't successful
    }

    console.info("Successfully fetched inbox messages:", response.data);

    return response.data; // Return the data (messages)
  } catch (error) {
    console.error("Failed to fetch inbox messages:", error);
    throw new Error("Failed to fetch inbox messages"); // Propagate error up the chain
  }
};
