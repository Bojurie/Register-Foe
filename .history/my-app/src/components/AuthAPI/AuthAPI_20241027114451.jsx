...
let hasShownNoReminderToast = false; // Tracks if the 'no reminders' message has been shown
...
const fetchReminder = async (userId) => {
  ...
  try {
    ...
    hasShownNoReminderToast = false; // Reset flag if a successful response is received
    return response.data;
  } catch (error) {
    if (error.response) {
      ...
      if (status === 404) {
        ...
        // Show the 'no reminders' message only once to avoid repetitive notifications
        if (!hasShownNoReminderToast) {
          toast.info("No reminders found for this user.");
          hasShownNoReminderToast = true;
        }

        return []; // Return an empty array to indicate no reminders
      } else if (status >= 500) {
