...
const postVote = async (candidateId) => {
  try {
    const response = await axiosInstance.post(
      `/election/elections/${election._id}/vote`,
      { candidateId }
    );
    return response.data;
  } catch (error) {
...
const getElectionsByTheirId = async ( electionId) => {
  ...
  try {
    const response = await axiosInstance.get(
      `/election/upcoming-elections/${electionId}`
    );
    return response.data;
  } catch (error) {
...
const GetUpComingElections = async (companyCode) => {
  ...
  try {
    // Fetch all elections for the given company code
