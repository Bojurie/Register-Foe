...
import {saveElectionError, saveElectionSeccess} from '../AuthContext/AuthContext'
...
const getAdminUsers = async (companyCode) => {
  if (!companyCode) {
    throw new Error("Company code is undefined or null");
  }
  try {
    const response = await axios.get(
      `/users/admin/byCompanyCode?companyCode=${companyCode}`
    );
    return response.data;
  } catch (error) {
    const errorMessage =
      error.response?.data?.error || "Failed to fetch admin users";
    throw new Error(`Error fetching admin users: ${errorMessage}`);
  }
};
...
// Thunk function to save election
const saveElection = (electionData) => async (dispatch) => {
  try {
    const response = await axiosInstance.post(
      "/savedElection/save-elections", 
      electionData
    );
    dispatch(saveElectionSuccess());
    enqueueSnackbar("Election saved successfully!", { variant: "success" });
  } catch (error) {
    dispatch(saveElectionError(error.message));
