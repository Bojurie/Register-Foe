...
const fetchReminder = async (userId, retryCount = 3) => {
  if (!userId) {
  ...
  try {
    ...
    if (error.response) {
      if (error.response.status === 404) {
        ...
        return []; // Return an empty array for no reminders
      } else if (error.response.status >= 500 && retryCount > 0) {
        console.warn(
          `Server error occurred. Retrying... ${retryCount} attempt(s) left`
        );
        return fetchReminder(userId, retryCount - 1);
      } else {
        toast.error("Failed to fetch reminders. Please try again later.");
      }
    } else {
