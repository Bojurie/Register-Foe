...
const getConversations = async (conversationId) => {
  try {
    console.log("Requesting messages for conversation ID:", conversationId); // Log the request details
    const response = await axiosInstance.get(
      `/message/messages/${conversationId}`
    ); // Ensure conversationId is used
    ...
    console.info("Messages fetched successfully:", messages);
    return messages;
  } catch (error) {
    console.error("Failed to fetch messages:", error); // Log any errors
