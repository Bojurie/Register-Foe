...
const GetUpComingElections = async (companyCode) => {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      console.log("Fetched elections from API:", elections);
...
const GetUpComingNews = async (companyCode) => {
  console.log("[INFO] Fetching news for company code:", companyCode);
  ...
  try {
    ...
    if (response.data && response.data.news) {
      console.log("[INFO] Successfully fetched news data:", response.data.news);
      return response.data;
    } else {
      console.warn("[WARN] No news data found in GetUpComingNews.");
