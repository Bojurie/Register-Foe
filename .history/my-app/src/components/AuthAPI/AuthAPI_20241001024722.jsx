...
const GetUpComingElections = async (companyCode) => {
  ...
  if (!companyCode) {
    console.warn("Election ID is missing:", companyCode);
    return null;
  }
  ...
  try {
    ...
    if (response.data && response.data.election) {
      console.info("Successfully fetched election with ID:", companyCode);
      return response.data.election;
    } else {
      console.warn("No election data found in response for ID:", companyCode);
      return null;
    }
  } catch (error) {
