...
const GetUpComingElections = async (companyCode, electionId) => {
  ...
  if (!companyCode || !electionId) {
    console.warn("Company code or election ID is missing:", {
      companyCode,
      electionId,
    });
    return null;
  ...
  try {
    // Make API request to fetch the election data
    const response = await axiosInstance.get(
    ...
    // Check if the response contains the required election data
...
const getMessages = async (userId) => {
  ...
  try {
    const response = await axiosInstance.get(
      `/message/message/messages/${userId}`
    );
