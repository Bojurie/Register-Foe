...
const GetUpComingNews = async (companyCode) => {
  ...
  try {
    ...
    if (response.data?.news) {
      console.log("[INFO] Successfully fetched the latest news data.");
      return response.data;
    } else {
      console.warn("[WARN] No news available.");
      return { news: [], message: "No news available." };
    }
  } catch (error) {
    console.error("[ERROR] Error fetching the latest news:", error.message);
...
const getCompanyNews = async (companyCode) => {
  try {
    return await GetUpComingNews(companyCode);
  } catch (error) {
