...
const GetUpComingElections = async (companyCode) => {
  ...
  try {
    // Perform an API call to retrieve the election details by company code
    const response = await axiosInstance.get(`/election/elections/${companyCode}`);

    console.info("API response for elections fetch:", response);

    // Check if response contains elections data
    if (response.data && response.data.elections) {
      console.info(
        "Successfully fetched elections for company code:",
        companyCode
      );
      return response.data.elections; // Return all fetched elections
    } else {
      console.warn(
