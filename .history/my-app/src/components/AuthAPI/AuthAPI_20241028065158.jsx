...
const GetUpComingNews = async (companyCode) => {
  ...
  try {
    ...
    console.error("[ERROR] Error fetching news:", error.message);
    return { news: [], message: "Failed to fetch updated news." };
  }
};
...
const MarkNewsAsReading = async (newsId) => {
  try {
    ...
    if (response.data?.success) {
      ...
      console.warn(
        "[WARN] Server responded with failure while marking as read."
      );
