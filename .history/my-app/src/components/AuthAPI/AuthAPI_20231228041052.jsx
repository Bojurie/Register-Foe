...
const GetUsersByCode = async () => {
  ...
  try {
    const response = await axios.get(`${BASE_URL}/company/companies/users`, {
      headers: {
...
const GetTopicsByCode = async (companyCode) => {
  try {
    const response = await axios.get(
      `${BASE_URL}/topic/topics?companyCode`
    );
    return response.data;
...
const GetUpComingElections = async () => {
  try {
    const response = await axios.get(`${BASE_URL}/election/upcoming-elections`);
    return response.data; // Returns the list of upcoming elections
  } catch (error) {
    console.error("Error fetching upcoming elections:", error);
    // It's often better to handle errors in the calling context for more flexibility.
    // However, rethrowing here to allow the caller to decide on the error handling.
    throw error;
  }
};
