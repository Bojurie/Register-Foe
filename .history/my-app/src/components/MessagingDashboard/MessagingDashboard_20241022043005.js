...
const MessagingDashboard = () => {
  ...
  const fetchAvailableUsers = useCallback(async () => {
    try {
      ...
      console.log("Fetched available users on component mount:", users); // Log available users
    } catch (error) {
  ...
  const fetchInboxMessagesHandler = useCallback(async () => {
    if (!selectedConversationId) {
      ...
      console.error("No conversation ID found when fetching inbox messages."); // Log error if no conversation ID
      return;
    }
    ...
    try {
      ...
      console.log("Fetched messages from server:", messages); // Log fetched messages

      setInboxMessages(messages || []);
    } catch (error) {
      setError("Failed to load inbox messages.");
      console.error("Error fetching inbox messages:", error); // Log error
    } finally {
  ...
  useEffect(() => {
    // Fetch messages only if there is a valid conversation ID
    if (activeTab === "Inbox" && selectedConversationId) {
      fetchInboxMessagesHandler();
    }
  }, [activeTab, selectedConversationId, fetchInboxMessagesHandler]);

  // Handle message click to open modal and fetch conversation messages
  const handleMessageClick = (message) => {
    if (!message?.conversation?._id) {
      console.error("Conversation ID is missing in the clicked message."); // Log error if conversation ID is missing
      return;
    }
    ...
    ); // Log clicked message's conversation ID
