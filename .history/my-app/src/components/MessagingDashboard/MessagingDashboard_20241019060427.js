...
const MessagingDashboard = () => {
  ...
  // Add the state for controlling dashboard visibility
  ...
  const handleMessageSend = async () => {
    ...
    try {
      await handleSendMessage(
        selectedConversation._id, // Ensure you pass the correct conversation or recipient ID
  ...
  // When clicking a message, set the conversation and show the reply box
  const handleMessageClick = (message) => {
    if (message.sender) {
      setSelectedConversation(message.sender); // Set the sender as the selected conversation
      setShowMessageInput(true); // Show the message input for replies
    } else {
      toast.error("Cannot reply to an unknown sender.");
    }
  };

  // Render the sender's name or fallback to "Unknown Sender"
  ...
  // Define the renderRecipients function to handle recipient display
                ...
                {inboxMessages.length > 0 ? (
                  inboxMessages.map((message) => (
                    <motion.div
                      ...
                      onClick={() => handleMessageClick(message)} // Handle click to show reply input
