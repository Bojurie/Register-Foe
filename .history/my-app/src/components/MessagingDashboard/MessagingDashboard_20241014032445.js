...
const handleSendMessage = async (conversationId, recipientId, content) => {
  // Ensure all fields are provided before attempting to send the message
  ...
  try {
    ...
    const message = await sendMessage(messageData); // Sending the message to the server
    return message;
  } catch (error) {
