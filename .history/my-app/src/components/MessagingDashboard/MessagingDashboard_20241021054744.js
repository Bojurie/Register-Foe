...
const MessagingDashboard = () => {
  ...
  const [inboxMessages, setInboxMessages] = useState([]); // Ensure this is initialized as an empty array
  ...
  const fetchAvailableUsers = useCallback(async () => {
    try {
      ...
      setAvailableUsers(users || []); // Default to an empty array if users are undefined
    } catch (error) {
      ...
      <AnimatePresence>
        {activeTab === "Inbox" && (
          <motion.div className="InboxMessagesContainer">
            ...
            ) : inboxMessages.length > 0 ? ( // Ensure inboxMessages is always an array before calling `.length`
