...
const Messages = ({ conversation }) => {
  const { fetchInboxMessages, handleSendMessage, user } = useAuth(); // Use 'user' from the auth context
  ...
  const fetchAllMessages = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetchInboxMessages(user._id); // Pass the user ID for fetching messages
