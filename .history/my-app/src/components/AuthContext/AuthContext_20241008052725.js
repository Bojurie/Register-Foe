...
const handleSendMessage = useCallback(
  async (conversationId, recipientId, content) => {
    ...
    try {
      ...
      const message = await sendMessage(messageData); // Call API
      return message;
    } catch (error) {
...
const addCommentToTopic = async (userId, topicId, comment) => {
  try {
    const topicData = { userId, topicId, comment };
    const response = await postComment(topicData); // Assuming postComment is your API call

    dispatch({
      type: ActionTypes.ADD_COMMENT_SUCCESS,
      payload: response.topic, // Assuming the response returns an updated topic with the new comment
    });

    enqueueSnackbar("Comment added successfully!", { variant: "success" });

    return response.comment; // Assuming the API returns the added comment separately
  } catch (error) {
