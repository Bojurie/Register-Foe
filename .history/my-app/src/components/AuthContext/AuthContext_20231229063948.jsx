...
export const AuthProvider = ({ children }) => {
  ...
  const fetchUserByCompanyCode = async (companyCode) => {
    ...
    try {
      ...
      return response; // Make sure this is always an object with a 'data' property
    } catch (error) {
      console.error("Error fetching users by company code:", error);
      return { data: [] }; // Return an object with an empty data array in case of an error
    }
  };
  ...
  return (
    <AuthContext.Provider value={(contextValue, handleCreateTopic)}>
      {children}
