...
useEffect(() => {
  const checkAuthentication = async () => {
    const token = getStoredToken(); // Make sure this function retrieves the token correctly

    if (token && (await validateToken())) {
      // Ensure validateToken makes a proper API call
      const storedUser = getStoredUser(); // Ensure this function retrieves the user data correctly
      dispatch({
        type: ActionTypes.LOGIN_SUCCESS,
        payload: { user: storedUser },
      });
    } else {
      removeStoredUser(); // Clear user data from storage
      removeStoredToken(); // Clear token from storage
      navigate("/login"); // Redirect to login page
    }

    dispatch({ type: ActionTypes.SET_LOADING, payload: { loading: false } }); // Set loading to false after check
  };
...
const handleLogin = useCallback(
  async (formData) => {
    try {
      const response = await axiosInstance.post("/login", formData); // Make API call to login
      const { token, user } = response.data;

      setStoredToken(token); // Store the token securely
      setStoredUser(user); // Store user data securely
