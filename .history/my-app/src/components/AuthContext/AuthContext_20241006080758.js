...
const fetchUserByCompanyCode = useCallback(async (companyCode) => {
 ...
 const getCandidatesById = useCallback(async (electionId) => {
   try {
     // Fetch the candidates using the fetchCandidatesById function
...
const fetchTopicsByCompanyCode = useCallback(async (companyCode) => {
  ...
  // POST A VOTE CANDIDATE 
    ...
    const postElectionVote = useCallback(async (candidateId) => {
      try {
        if (!candidateId) {
          console.log("User ID is undefined or null");
          return { data: [] };
        }
        const response = await postVote(candidateId);
        return response;
      } catch (error) {
...
const fetchSavedElections = useCallback(async (userId) => {
 ...
 const contextValue = useMemo(() => {
  const { user, electionCreated, electionError, isElectionSaved, saveElectionError } = state;

  return {
    user,
    userId: user?._id, // Adding a userId directly for easy reference
  ...
  state.user,
  state.electionCreated,
  state.electionError,
  state.isElectionSaved,
  state.saveElectionError,
  // Only include functions that depend on state values
  handleLogout,
  handleLogin,
  handleSaveElection,
  getElections,
  ...
  fetchConversationMessages, // Added this function to dependencies
]);
