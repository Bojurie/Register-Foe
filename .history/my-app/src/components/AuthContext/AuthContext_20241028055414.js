...
const markAsReadNews = async (newsId) => {
  try {
    ...
    if (response?.success) {
      console.log(`[INFO] Marked news item ${newsId} as read on the backend.`);
      return {
        success: true,
        message: response.message,
        data: response.newsItem,
      };
    } else {
      console.warn("[WARN] Unexpected response format from markAsRead.");
      return {
        success: false,
    ...
    return {
      success: false,
      message: "An error occurred while marking the news as read.",
    };
  }
};
