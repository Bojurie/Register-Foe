...
const getElections = useCallback(async (companyCode, electionId) => {
  // Check if both companyCode and electionId are provided
  if (!companyCode || !electionId) {
    console.warn("Invalid company code or election ID provided:", {
      companyCode,
      electionId,
    });
    return null;
  ...
  try {
    // Call GetUpComingElections and store response
    const election = await GetUpComingElections(companyCode, electionId);

    // Check if election was successfully fetched
    if (!election) {
      console.warn("Election not found with the provided ID:", electionId);
      return null;
    }
