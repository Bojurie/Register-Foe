...
const handledeleteReminder = async (formData) => {
  try {
    const response = await deleteReminder(formData);
    enqueueSnackbar("Reminder created successfully!", { variant: "success" });
    return response;
  } catch (error) {
    const errorMessage =
...
const getUserReminder = async ({ userId }) => {
  try {
    ...
    console.log("User Reminder Data:", response); // Log the data
  ...
  const getElections = useCallback(async (companyCode) => {
    try {
      if (!companyCode) {
        console.log("Election by CompanyCode received companyCode:", companyCode);
        return { data: [] };
      }
...
const fetchSavedElections = useCallback(async (userId) => {
  try {
    ...
    return { savedElections: response }; // Ensure response is an array
  } catch (error) {
  ...
  async (electionId) => {
    try {
      ...
      if (response.success) {
        ...
        const errorMessage =
          response.error || "An unknown error occurred while saving elections";
