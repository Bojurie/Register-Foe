...
const fetchLikesAndDislikes = async (topicId, action) => {
  try {
    console.log(`Fetching ${action} for topic ID: ${topicId}`);

    // Send the request and log the response
    const response = await getReactionsNow(topicId, action);
    console.log(`Response received for ${action}:`, response);

    // Determine if the response includes likes or dislikes based on the action
    const count =
      action === "likes" ? response.likeCount : response.dislikeCount;
    console.log(`Count of ${action} for topic ID ${topicId}:`, count);

    if (count !== undefined) {
      console.log(`Successfully fetched ${action} count:`, count);
      ...
      // Return both count and list of users and log the users array
      console.log(`${action} list of users:`, response[action]);
      return { count, users: response[action] };
    } else {
      // Log the error if count is missing
