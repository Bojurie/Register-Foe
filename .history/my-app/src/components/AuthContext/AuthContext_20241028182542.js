...
const markAsReadNews = async (newsId) => {
  try {
    ...
    if (response?.success) {
      ...
      return {
        success: false,
        message: response?.message || "Failed to mark as read on the server.",
      };
    }
  } catch (error) {
    const errorMessage =
...
const getCompanyNews = async (companyCode) => {
  console.log("[INFO] Fetching company news for code:", companyCode);
  try {
    const response = await GetUpComingNews(companyCode); // Calls the GetUpComingNews function directly
    if (response?.news?.length > 0) {
      console.log("[INFO] Company news fetched successfully:", response.news);
