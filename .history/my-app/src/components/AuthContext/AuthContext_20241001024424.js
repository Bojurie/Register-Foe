...
const getUserReminder = async (userId) => {
  if (!isValidObjectId(userId)) {
    console.warn("Invalid user ID for reminders:", userId);
...
// Validate that companyCode is not null or empty
const isValidCompanyCode = (code) =>
  code && typeof code === "string" && code.trim() !== "";

// Fetch all upcoming elections by company code
const getElectionsByCompanyCode = useCallback(async (companyCode) => {
  ...
  try {
    // Perform an API call to retrieve all elections associated with the company code
    const response = await axiosInstance.get(`/elections/${companyCode}`);

    console.info("API response for elections fetch by company code:", response);
    ...
    if (response.data && response.data.elections) {
      ...
      console.warn(
        "No elections data found in response for company code:",
        companyCode
      );
      return null;
    }
  } catch (error) {
    handleApiError(error, "Failed to fetch upcoming elections by company code");
    return null;
  }
}, []);
