...
const markAsReadNews = async (newsId) => {
  try {
    ...
    if (response?.success && response.newsItem) {
      console.log("[INFO] News item marked as read:", response.newsItem);
      return { success: true, newsItem: response.newsItem };
    }
    return { success: false, message: "Failed to mark news as read." };
  } catch (error) {
    const errorMessage =
      error.message || "Unexpected error marking news as read.";
    console.error("[ERROR] Failed to mark news as read:", errorMessage);
...
const getCompanyNews = async (companyCode) => {
  console.log(
    "[INFO] Fetching company news with getCompanyNews for code:",
    companyCode
  );
  try {
    const response = await GetUpComingNews(companyCode);
    if (response?.news?.length > 0) {
      console.log("[INFO] Retrieved company news successfully:", response.news);
      return response;
    } else {
