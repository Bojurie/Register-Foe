...
const fetchAdminUsers = useCallback(async (companyCode) => {
  ...
  const handleCreateCompanyNews = async (topicData) => {
    try {
      const user = getStoredUser();
      if (!user) {
        enqueueSnackbar("You must be logged in to create a topic.", {
      ...
      const response = await createCompanyNews(topicData);
      dispatch({
        type: ActionTypes.CREATE_TOPIC_SUCCESS,
        payload: response.topic,
      });
      ...
      handleCreationCompanyNewsError(error);
    }
  };
  const handleCreationCompanyNewsError = (error) => {
    const errorMessage =
      error.response?.data?.error || "Failed to create topic.";
    dispatch({
      type: ActionTypes.CREATE_TOPIC_ERROR,
    ...
    try {
      ...
      dispatch({
        type: response.success
          ? ActionTypes.SAVE_ELECTION_SUCCESS
          : ActionTypes.SAVE_ELECTION_ERROR,
        payload: { error: response.success ? "" : "Failed to save election" },
      });
      enqueueSnackbar(
        response.success
      ...
      dispatch({
        ...
        payload: { error: "An error occurred while saving the election" },
      });
      enqueueSnackbar("An error occurred while saving the election", {
        variant: "error",
      });
    }
  };
