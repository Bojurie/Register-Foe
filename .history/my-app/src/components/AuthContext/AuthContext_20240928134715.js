...
const getElections = useCallback(async (companyCode, electionId) => {
  try {
    if (!electionId || !companyCode) {
      console.log(
        "Invalid company code or election ID:",
        companyCode,
        electionId
      );
      return null; // Return null when companyCode or electionId is not provided
    }

    const election = await GetUpComingElections(companyCode, electionId);
    if (!election) {
      console.warn("No election found with the given ID:", electionId);
      return null;
    }
    return election;
  } catch (error) {
