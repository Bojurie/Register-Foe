...
import * as AuthAPI from "../AuthAPI/AuthAPI";
import * as LocalStorageManager from "../LocalStorageManager/LocalStorageManager";
...
export const AuthProvider = ({ children }) => {
  const [state, dispatch] = useReducer(authReducer, {
    user: LocalStorageManager.getStoredUser(),
  });
  const navigate = useNavigate();
  const { enqueueSnackbar } = useSnackbar();

  useEffect(() => {
    const token = LocalStorageManager.getStoredToken();
    if (token) {
