...
const ActionTypes = {
  ...
  UPDATE_LIKES_SUCCESS: "UPDATE_LIKES_SUCCESS", // Added
  UPDATE_DISLIKES_SUCCESS: "UPDATE_DISLIKES_SUCCESS", // Added
...
const authReducer = (state, action) => {
  switch (action.type) {
    ...
    case ActionTypes.UPDATE_DISLIKES_SUCCESS: {
      const updatedTopics = Array.isArray(state.topics)
        ? state.topics.map((topic) =>
            topic._id === action.payload.topicId
              ? {
...
const updateTopicVote = useCallback(async (userId, topicId, action) => {
  ...
  try {
    ...
    showMessage(response.message || "Vote registered successfully!", "success");

    dispatch({
      type: ActionTypes.UPDATE_VOTE_SUCCESS,
      payload: {
        topicId,
        likesCount: response.reactions.likeCount,
        dislikesCount: response.reactions.dislikeCount,
      },
    });
