...
const fetchUserByCompanyCode = useCallback(async (companyCode) => {
  ...
  try {
    ...
    if (users.length > 0) {
      console.info(
        `Fetched ${users.length} users for company code: ${companyCode}`
      );
      return users;
    } else {
...
const fetchTopicsByCompanyCode = useCallback(async (companyCode) => {
  ...
  try {
    ...
    if (topics && topics.length > 0) {
      console.info(
        `Fetched ${topics.length} topics for company code:`,
        companyCode
      );
      return topics;
    } else {
...
const fetchPastElection = useCallback(async (companyCode) => {
  ...
  try {
    console.log("Fetching past elections for company code:", companyCode);
    const response = await getPastElections(companyCode);

    if (response && response.length > 0) {
      console.info(
        `Fetched ${response.length} past elections for company code:`,
        companyCode
      );
      return response;
    } else {
      console.warn(`No past elections found for company code: ${companyCode}`);
      return []; // Gracefully return an empty array when no elections are found
    }
  } catch (error) {
...
const fetchInboxMessages = useCallback(async (conversationId) => {
  try {
    const conversations = await getConversations(conversationId);
