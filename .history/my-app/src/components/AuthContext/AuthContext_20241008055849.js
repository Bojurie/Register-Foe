...
const getSentMessages = useCallback(async () => {
  ...
  const updateTopicVote = async (userId, topicId, action) => {
    ...
    const addCommentToTopic = useCallback(async (topicId, comment) => {
    ...
    try {
      ...
      dispatch({
        ...
        payload: response.comments, // Assuming comments array is returned
      });
  ...
  }, [state.user, dispatch, enqueueSnackbar]);
