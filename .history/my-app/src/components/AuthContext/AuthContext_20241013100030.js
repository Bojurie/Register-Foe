...
const handleAuthentication = useCallback(async () => {
  ...
  // Ensure the user exists and has a valid ID
  if (!user || !user._id) {
  ...
  // If everything is valid, dispatch LOGIN_SUCCESS
  dispatch({ type: ActionTypes.LOGIN_SUCCESS, payload: { user } });

  // Redirect user to main page
  navigate("/main"); // Add this if you want automatic redirection after authentication
}, [dispatch, navigate]);
...
//       return { data: [] };
//     }
//     const pastElections = await getPastElections(userId);
