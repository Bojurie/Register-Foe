...
export const AuthProvider = ({ children }) => {
  ...
  const postUserReminder = async (formData) => {
    dispatch({ type: ActionTypes.REQUEST_REMINDER }); // Start loading state

    try {
      const response = await newReminder(formData); // Call to post new reminder
      toast.success("Reminder created successfully!"); // Success toast
      dispatch({
        type: ActionTypes.REMINDER_SUCCESS, // Dispatch success action
        payload: response,
      });
      return response; // Return response from server
    } catch (error) {
      dispatch({
        type: ActionTypes.REMINDER_ERROR, // Dispatch error action
        payload: error.response?.data?.error || "Failed to create reminder",
      });
      toast.error(error.response?.data?.error || "Failed to create reminder"); // Error toast
...
const getUserReminder = async ({ userId }) => {
  ...
  try {
    ...
    return reminders || []; // Ensure returning an array even if empty
  } catch (error) {
    const errorMessage =
...
// DELETE THIS LATER IT FOR SAVE ELECTION
  ...
  const fetchInboxMessages = useCallback(
    async (conversationId) => {
      ...
      try {
        setLoading(true);
        const { conversations, messages } = await getConversations(); // Fetch conversations and messages

        console.log("Conversations from getConversations:", conversations);
        console.log("Messages from getConversations:", messages);
        ...
        if (conversationMessages.length > 0) {
          console.log("Messages set successfully:", conversationMessages);
          return conversationMessages; // Return filtered messages
        } else {
  ...
  // MARK NEWS ARE READ
const markAsReadNews = async (newsId, userId, enqueueSnackbar) => {
  try {
    const response = await getNewsReading(newsId, userId);
    enqueueSnackbar(response.message || "News marked as read successfully!", {
      variant: "success",
    });
    return response; // Return response in case further actions are needed
  } catch (error) {
    enqueueSnackbar(
      error.message || "Failed to mark news as read. Please try again.",
      { variant: "error" }
...
handleCreateTopic,
  ...
  return (
    <AuthContext.Provider value={contextValue}> {state.authChecking ? null : children}</AuthContext.Provider>
  );
};
