...
const GetTopicsByCode = async (companyCode) => {
  try {
    const token = localStorage.getItem("token"); // Ensure this is the correct key for your stored token
...
const fetchTopicsByCompanyCode = async (companyCode) => {
  try {
    return await GetTopicsByCode(companyCode); // Directly return the result of GetTopicsByCode
  } catch (error) {
    console.error("Error fetching topics:", error);
    throw error;
  }
