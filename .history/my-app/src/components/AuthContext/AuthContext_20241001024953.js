...
// Function to get elections by company code
const getElectionsByCompanyCode = useCallback(async (companyCode) => {
  ...
  try {
    // Fetch all elections associated with the provided company code
    const elections = await GetUpComingElections(companyCode);

    // Check if elections were successfully fetched
    ...
    console.info(
      "Elections successfully retrieved for company code:",
      companyCode,
      elections
    );
