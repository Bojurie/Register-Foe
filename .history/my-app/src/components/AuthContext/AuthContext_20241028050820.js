...
const markAsReadNews = async (newsId) => {
  try {
    ...
    if (response?.success) {
      ...
      console.warn(
        "Unexpected response format when marking news as read:",
        response
      );
      return {
        success: false,
    ...
    console.error("Failed to mark news as read:", error.message || error);
    return {
      success: false,
      message:
        error.message || "An error occurred while marking the news as read.",
    };
  }
};
