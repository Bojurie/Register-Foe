...
export const AuthProvider = ({ children }) => {
  const [state, dispatch] = useReducer(authReducer, {
    user: getStoredUser(), // Retrieves the initial user state
  });
  ...
  const { enqueueSnackbar } = useSnackbar();
   ...
   const processLoginResponse = useCallback(
     (response) => {
       ...
       console.log("Token stored:", response.token); // Check if the token is being stored
  ...
  // Function to process login response
