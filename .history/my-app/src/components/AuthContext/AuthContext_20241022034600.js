...
export const AuthProvider = ({ children }) => {
  ...
  const handleAuthentication = useCallback(async () => {
    ...
    console.log("Stored user:", user); // Log user to verify
  ...
  const getElections = useCallback(async (companyCode) => {
    ...
    try {
      ...
      return elections; // Return the fetched elections
    } catch (error) {
  ...
  const fetchPastElection = useCallback(async (companyCode) => {
    ...
    try {
      ...
      if (response && response.length > 0) {
        ...
        console.warn(
          `No past elections found for company code: ${companyCode}`
        );
  ...
  const getCompanyNews = useCallback(async (companyCode) => {
    try {
      if (!companyCode) {
        console.warn("getCompanyNews received an empty companyCode");
        return { news: [], message: "No company code provided." };
      }
      ...
      if (response && response.news && response.news.length > 0) {
        ...
        return { news: [], message: "No news found for this company." };
      }
    } catch (error) {
  ...
  // Function to fetch inbox messages for a specific conversation ID
  const fetchInboxMessages = useCallback(
    async (conversationId) => {
      ...
      console.log("Fetching messages for conversation ID:", conversationId); // Log the conversation ID

      try {
        setLoading(true);
        // Fetch messages using the conversation ID
        const response = await getConversations(conversationId);
        console.log("Fetched messages from server:", response); // Log the server response

        if (Array.isArray(response) && response.length > 0) {
          console.log("Messages set successfully:", response);
          return response; // Return the fetched messages
        } else {
        ...
        console.error("Error fetching inbox messages:", error); // Log the error for debugging
        return [];
      } finally {
        setLoading(false); // Ensure loading is stopped once the operation is complete
      }
    },
    [getConversations]
  );

  // Function to handle fetching inbox messages and setting state
  const fetchInboxMessagesHandler = useCallback(async () => {
    const conversationId = selectedConversationId; // Ensure the correct conversation ID is passed here

    if (!conversationId) {
      setError("Conversation not found.");
      console.error("Error: Conversation ID is missing."); // Log an error if the conversation ID is not provided
      return;
    }

    console.log("Fetching messages for conversation ID:", conversationId); // Log the conversation ID being used

    try {
      setLoadingMessages(true);
      // Fetch messages based on the conversation ID
      const messages = await fetchInboxMessages(conversationId);
      console.log("Fetched messages from server:", messages); // Log the fetched messages

      setInboxMessages(messages || []); // Set the fetched messages to the state
    } catch (error) {
      setError("Failed to load inbox messages.");
      console.error("Error fetching inbox messages:", error); // Log the error for debugging
    } finally {
      setLoadingMessages(false); // Ensure the loading state is updated
    }
