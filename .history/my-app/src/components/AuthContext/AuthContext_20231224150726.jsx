...
const handleLogin = useCallback(
  async (formData) => {
    try {
      ...
      const { token, user } = response.data; // Destructure user from response payload

      setStoredUser(user); // Store the user object received from the server
      setStoredToken(token); // Store the token
