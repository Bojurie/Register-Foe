...
const markAsReadNews = async (newsId) => {
  try {
    ...
    return response?.newsItem
      ? { success: true, newsItem: response.newsItem }
      : null;
  } catch (error) {
    console.error("[ERROR] Failed to mark news as read:", error.message);
...
const getCompanyNews = async (companyCode) => {
  if (!companyCode) {
    console.warn("[WARN] getCompanyNews received an empty companyCode.");
