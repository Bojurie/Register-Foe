...
const getElections = useCallback(async (electionId) => {
  ...
  try {
    const response = await GetUpComingElections(electionId);
    if (!response) {
      console.warn("Election not found with the provided ID:", electionId);
      return null;
    }
    console.info("Election successfully retrieved:", response);
    return response;
  } catch (error) {
    handleApiError(error, "Error while fetching elections");
    return null;
  }
}, []);
...
const fetchSavedElections = useCallback(async (userId) => {
  ...
  try {
    ...
    handleApiError(error, "Error in fetchSavedElections");
    return [];
  }
}, []);
