...
const handleRegister = async (formData) => {
  try {
    ...
    for (let key in formData) {
      console.log(`Appending key: ${key}, value: ${formData[key]}`); // Debug: Log each key-value being appended
      dataToSend.append(key, formData[key]);
    }

    console.log("Data to send:", dataToSend); // Debug: Log FormData before sending
...
const updateProfileLike = async (userId, electionId) => {
  try {
    ...
    dispatch({
      ...
      payload: { userId, likesCount: response.likesCount },
    });
  } catch (error) {
  ...
  return (
    <AuthContext.Provider value={contextValue} value={{ updateRoleToAdmin }}>
      {children}
