...
export const AuthProvider = ({ children }) => {
  ...
  // SEND A MESSAGE
const handleSendMessage = useCallback(async (recipientId, content) => {
  // Log the recipientId and content values before sending
  ...
  try {
    const messageData = { recipientId, content: content.trim() }; // Trim the content
    console.log("Sending message with data:", messageData); // Log the message data
  ...
  const fetchInboxMessages = useCallback(
    async (userId) => {
      ...
      try {
        ...
        console.log("Fetched inbox messages:", response);

        if (Array.isArray(response) && response.length > 0) {
          console.log("Messages set successfully:", response);
          return response; // Return messages here
        } else {
