...
const markAsReadNews = async (newsId) => {
  try {
    ...
    if (response?.success) {
      console.log(
        "[INFO] Successfully marked news as read in the backend:",
        response.newsItem
      );
      ...
      return {
        success: false,
        message: "Failed to mark as read on the server.",
      };
    }
  } catch (error) {
    const errorMessage =
...
const getCompanyNews = async (companyCode) => {
  ...
  if (!companyCode) {
    console.warn("[WARN] No company code provided to getCompanyNews.");
