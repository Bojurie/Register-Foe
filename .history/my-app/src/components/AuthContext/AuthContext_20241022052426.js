...
const fetchInboxMessages = useCallback(
  async (conversationId) => {
    if (!conversationId) {
      ...
      console.error("Conversation ID is missing."); // Log missing conversation ID
      return [];
    }

    console.log("Fetching messages for conversation ID:", conversationId); // Log the provided conversation ID

    try {
      setLoading(true);
      const { conversations, messages } = await getConversations(); // Fetch both conversations and messages

      console.log("Conversations from getConversations:", conversations); // Log fetched conversations
      console.log("Messages from getConversations:", messages); // Log fetched messages

      if (!conversations || conversations.length === 0) {
        setError("No conversations found.");
        console.warn("No conversations returned from the server."); // Log missing conversations
        setLoading(false);
        return [];
      }
      ...
      console.log("Filtered messages for conversation:", conversationMessages); // Log filtered messages

      if (conversationMessages.length > 0) {
        console.log("Messages set successfully:", conversationMessages);
        return conversationMessages; // Return the filtered messages
      } else {
        setError("No messages found for this conversation.");
        console.warn("No messages found for conversation ID:", conversationId); // Log if no messages were found
        return [];
      }
    } catch (error) {
