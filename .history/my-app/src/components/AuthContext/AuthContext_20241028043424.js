...
export const AuthProvider = ({ children }) => {
  ...
  // MARK NEWS ARE READ
const markAsReadNews = async (newsId, enqueueSnackbar) => {
  try {
    const response = await getNewsReading(newsId);

    if (response?.success) {
      enqueueSnackbar("News marked as read successfully!", {
        variant: "success",
      });
      return response;
    } else {
      enqueueSnackbar("Failed to update read status on server.", {
        variant: "warning",
      });
      return null;
    }
  } catch (error) {
    enqueueSnackbar("Failed to mark news as read. Please try again.", {
      variant: "error",
    });
