...
const handleLogin = useCallback(
  async (formData) => {
    console.log("Logging in..."); // Add this line for debugging

    try {
      // Call the login API to get the user and token
      const { token, user } = await login(formData);
      console.log("Received token in handleLogin:", token);
      console.log("Received user in handleLogin:", user); // Log the user object

      // Set the user in the context
      setStoredToken(token);

      // Log the user object before setting it in local storage
      console.log("User object before setting in local storage:", user);

      setStoredUser(user); // Set the user in local storage
      dispatch({ type: ActionTypes.LOGIN_SUCCESS, payload: { user } });
