...
export const AuthProvider = ({ children }) => {
  ...
  const fetchUserByCompanyCode = useCallback(
    async (companyCode) => {
    if (!companyCode) {
 ...
 const fetchTopicsByCompanyCode = useCallback(
    async (companyCode) => {  if (!companyCode) {
  console.log("fetchTopicsByCompanyCode received companyCode:", companyCode);
    return { data: [] };
  }
  try {
...
[]
  ...
  const updateRoleToAdmin = async (userId) => {
    try {
      // Assuming updateUserRole is an API call that updates the user's role
