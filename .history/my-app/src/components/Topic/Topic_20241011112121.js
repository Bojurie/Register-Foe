...
const Topic = ({ topic }) => {
  ...
  useEffect(() => {
    const fetchReactions = async () => {
      try {
        console.log(`Fetching likes from: /vote/topics/${topic._id}/likes`); // Log likes request URL
        const likesResponse = await axiosInstance.get(
          `/vote/topics/${topic._id}/likes`
        );
        console.log("Likes response:", likesResponse.data); // Log likes response

        console.log(
          `Fetching dislikes from: /vote/topics/${topic._id}/dislikes`
        ); // Log dislikes request URL
        const dislikesResponse = await axiosInstance.get(
          `/vote/topics/${topic._id}/dislikes`
        );
        console.log("Dislikes response:", dislikesResponse.data); // Log dislikes response

        console.log(
          `Fetching comments from: /vote/topics/${topic._id}/comments`
        ); // Log comments request URL
