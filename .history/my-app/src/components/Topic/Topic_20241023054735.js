...
const ReactionButton = styled(IconButton)({
  ...
  "& svg": { color: "#657103", marginRight: "4px" },
});
...
const CommentButton = styled(IconButton)({
  ...
  "& svg": { color: "#f5a623", marginRight: "4px" },
});
...
const DeleteButton = styled(IconButton)({
  ...
  "& svg": { color: "#ff3333", marginRight: "4px" },
});
...
const Topic = ({ topic }) => {
  ...
  const { enqueueSnackbar } = useAuth();
  const fetchedDataRef = useRef({ likesFetched: false, dislikesFetched: false });

  const canDelete = user?.role === "admin" || (user?.isCompany && user?.companyCode === topic.companyCode);

  useEffect(() => {
    const fetchCommentsCount = async () => {
      try {
        const response = await axiosInstance.get(`/vote/topics/${topic._id}/comments/comment`);
        setCommentsCount(response.data.comments.length);
      } catch {
        enqueueSnackbar("Failed to fetch comments count.", { variant: "error" });
      }
    };
    ...
    const fetchReactions = async () => {
      try {
        ...
        if (!fetchedDataRef.current.dislikesFetched) {
          const dislikesResponse = await fetchLikesAndDislikes(topic._id, "dislikes");
          setDislikesCount(dislikesResponse?.count || 0);
          fetchedDataRef.current.dislikesFetched = true;
        }
      } catch {
        enqueueSnackbar("Failed to fetch likes or dislikes.", { variant: "error" });
      }
    };
