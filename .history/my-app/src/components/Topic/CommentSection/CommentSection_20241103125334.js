      ...
      <Box mt={2}>
        {loading ? (
          ...
          comments.map((comment) => (
            <Box key={comment.id} mb={3} borderBottom="1px solid #ddd" pb={2}>
              ...
              <Box display="flex" alignItems="center" mt={1}>
                ...
                <IconButton onClick={() => toggleReplyInput(comment.id)}>
                  <FaReply />
                </IconButton>
              </Box>
              ...
              {expandedReplies[comment.id] && (
                ...
                >
                  {comment.replies?.map((reply) => (
                    <Box key={reply.id} mb={2}>
                      ...
                      <Box display="flex" alignItems="center">
                        <Button
                          onClick={() => handleVote(reply.id, "like", true)}
                        >
                          <FaThumbsUp /> {reply.likesCount || 0}
                        </Button>
                        <Button
                          onClick={() => handleVote(reply.id, "dislike", true)}
                        >
                  ...
                  {replyInputVisible[comment.id] && (
                    <Box>
                      ...
                      <Button
                        ...
                        variant="contained"
                        sx={{ mt: 1 }}
                        disabled={!replies[comment.id]?.trim()}
                      >
                        Submit Reply
                      </Button>
                    </Box>
                  ...
                  <Button onClick={() => toggleReplyInput(comment.id)}>
                    {replyInputVisible[comment.id] ? "Cancel" : "Reply"}
                  </Button>
                </Box>
              )}
            </Box>
