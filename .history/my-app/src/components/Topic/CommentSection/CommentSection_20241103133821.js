...
const CommentSection = ({ topicId }) => {
  ...
  const handleError = (error) => {
    ...
    const errorMessage =
      error.response && error.response.status === 404
        ? "The comments for this topic were not found."
        : "Failed to load comments. Please check your internet connection and try again.";
  ...
  const handleVote = async (id, reaction) => {
    ...
    const updatedComments = comments.map((comment) => {
      if (comment.id && comment.id.$oid === id) {
        const updatedComment = { ...comment };
        // Safely increment likeCount or dislikeCount
        if (updatedComment.reactions) {
          if (reaction === "like") {
            updatedComment.reactions.likeCount =
              (updatedComment.reactions.likeCount || { $numberInt: 0 })
                .$numberInt + 1;
          } else if (reaction === "dislike") {
            updatedComment.reactions.dislikeCount =
              (updatedComment.reactions.dislikeCount || { $numberInt: 0 })
                .$numberInt + 1;
          }
        }
  ...
  const handleCommentSubmit = async (commentId = null) => {
    ...
    try {
      ...
      fetchComments();
      if (!commentId) setNewComment(""); // Clear input for new comments
    } catch (error) {
      console.error(
        ...
        {loading ? (
          ...
          <>
            ...
            {comments.map((comment) => {
              const commentId = comment.id ? comment.id.$oid : null; // Safe access
              if (!commentId) {
