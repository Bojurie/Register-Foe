...
const CommentSection = ({ topicId }) => {
  ...
  const handleCommentSubmit = async (commentId = null, action = null) => {
    const endpoint = commentId
      ? `/vote/vote/topics/${topicId}/comments/${commentId}/${action || "reply"}`
            ...
            <Box key={comment._id} mb={2} borderBottom="1px solid #ccc" pb={2}>
              ...
              <Box display="flex" alignItems="center" mt={1}>
                ...
                <IconButton onClick={() => toggleReplies(comment._id)}>
                  {expandedReplies[comment._id] ? <ExpandLess /> : <ExpandMore />}
                </IconButton>
                ...
                <Collapse in={expandedReplies[comment._id]} timeout="auto" unmountOnExit>
                  ...
                  <Box mt={2} pl={2} maxHeight="200px" overflow="auto">
                    {comment.replies?.map((reply) => (
                      <Box key={reply._id} mb={1} pb={1} borderBottom="1px solid #eee">
                        ...
                        <Box display="flex" alignItems="center" mt={1}>
                          ...
                          <IconButton onClick={() => handleReplyChange(reply._id, "")}>
                            <FaReply />
                          </IconButton>
                        </Box>
                      </Box>
