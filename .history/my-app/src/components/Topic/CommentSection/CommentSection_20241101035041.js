...
const CommentSection = ({ topicId }) => {
  ...
  const handleReplySubmit = async (commentId) => {
    ...
    try {
      ...
      // Immediately add the reply to the comment in the state
      setComments((prev) =>
        prev.map((comment) =>
      ...
      // Optionally call getHandleReply to refresh the reply data
      getHandleReply(commentId);
...
const getHandleReply = async (commentId) => {
  try {
    const response = await axiosInstance.get(
      `/vote/topics/${topicId}/comments/${commentId}/reply`
    );

    // Log the response to see the entire data returned
    console.log("Response from the server:", response);

    // Assuming the response has the structure you expect
    setComments((prev) =>
      prev.map((comment) =>
        comment._id === commentId
          ? { ...comment, replies: response.data.replies } // Update replies
          : comment
      )
    );

    // Clear the reply text after successful fetch
    setReplyText((prev) => ({ ...prev, [commentId]: "" }));
    setMessage({ type: "success", text: "Replies fetched successfully!" });
  } catch (error) {
    // Log the error to see what went wrong
    console.error("Error fetching replies:", error);
    setMessage({ type: "error", text: "Failed to fetch replies." });
  }
};
          ...
          <CommentBox key={comment._id}>
            ...
            {replyText[comment._id] !== undefined && (
              <Box className="ReplyBox" mt={1}>
                ...
                {/* Displaying replies below the comment */}
                <Box mt={2}>
                  {comment.replies &&
