...
const CommentSection = ({ topicId, comments = [], updateComments }) => {
  ...
  // Log the comments to debug their state
  console.log("Comments passed to CommentSection:", comments);
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      if (response.data.comment) {
        ...
        setMessage({
          type: "info",
          text: "Comment added, but not updated in the comments list.",
        });
      }
  ...
  const handleReaction = async (commentId, type, isReply = false) => {
    try {
      ...
      if (isReply) {
        ...
        const updatedComment = { ...response.data.reactions, _id: commentId };
        updateComments((prev) =>
          prev.map((comment) =>
