...
const CommentSection = ({ topicId }) => {
  ...
  const handleCommentSubmit = async (commentId = null, action = null) => {
    // Determine the endpoint based on whether it's a reply or a new comment
    ...
    try {
      await axiosInstance.post(endpoint, { text, action });
      if (commentId) {
        setReplies((prev) => ({ ...prev, [commentId]: "" })); // Clear the reply input after submission
      } else {
        setNewComment(""); // Clear the input field after submitting a new comment
      }
