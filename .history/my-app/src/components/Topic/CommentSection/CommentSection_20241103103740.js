...
const handleVote = (commentId, voteType) => {
  // Ensure we are sending the correct action type (like/dislike)
  handleCommentSubmit(commentId, voteType);
};
...
const handleCommentSubmit = async (commentId = null, action = null) => {
  ...
  // Ensure action is present if commentId is provided
  if (commentId && !action) {
    console.error("Action is required when providing a comment ID.");
    return; // Prevent sending request if action is not defined
  }
