  ...
  const handleVote = async (commentId, reaction) => {
    ...
    const updatedComments = comments.map((comment) => {
      if (comment.id === commentId) {
        ...
        // Ensure reactions object exists before accessing its properties
        ...
        // Handle already liked comments
        if (
          reaction === "like" &&
          updatedComment.reactions.likes.includes(userId)
        ) {
          setError("You can only like a comment once.");
          return updatedComment; // Do not update if already liked
