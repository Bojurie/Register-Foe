  ...
  const fetchComments = async () => {
    try {
      ...
      if (response.data.comments) {
        ...
        setComments([]); // Initialize as an empty array if comments are undefined
      }
      setMessage({ type: "success", text: "Comments loaded successfully!" });
    } catch {
      setMessage({ type: "error", text: "Failed to load comments." });
      setComments([]); // Also initialize to an empty array on error
    }
  };
  fetchComments();
}, [topicId]);
...
const handleReaction = async (commentId, type, isReply = false) => {
  try {
    ...
    if (isReply) {
      // Check for the parent comment first
      ...
      if (parentComment) {
        const updatedReplies = parentComment.replies.map((reply) => {
          // Ensure we are updating the correct reply
