...
const CommentSection = ({ topicId }) => {
  ...
  const handleCommentSubmit = async (commentId = null, action = null) => {
    const endpoint = commentId
      ? `/vote/vote/topics/${topicId}/comments/${commentId}` // Removed action from URL for replies
      : `/vote/vote/topics/${topicId}/comments`;
    ...
    console.log("Submitting comment:", { endpoint, text, action });

    try {
      const requestBody = {
        text: action ? undefined : text,
        action: action || undefined,
      }; // Include action if defined
