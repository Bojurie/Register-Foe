...
const CommentSection = ({ topicId }) => {
        ...
        if (comment.id === commentId) {
          return {
            ...
            reactions: {
              ...comment.reactions,
              [reaction + "Count"]:
                (comment.reactions[reaction + "Count"] || 0) + 1,
            },
          };
        }
        if (comment.replies) {
          return { ...comment, replies: updateCommentsList(comment.replies) };
        }
        return comment;
      });
  ...
  const handleCommentSubmit = async (commentId = null) => {
    const requestBody = {
      text: commentId ? replyInputs[commentId]?.trim() : newComment.trim(),
    };
    ...
    try {
      ...
      if (commentId) setReplyInputs((prev) => ({ ...prev, [commentId]: "" }));
    } catch (error) {
      console.error(
  ...
  const renderComment = (comment) => {
    const commentId = comment.id || comment._id?.$oid;
    if (!commentId) {
    ...
    return (
      <Box key={commentId} mb={3} borderBottom="1px solid #ddd" pb={2}>
        ...
        <Box display="flex" alignItems="center" mt={1}>
          ...
          <IconButton onClick={() => handleReplyChange(commentId, "")}>
            <FaReply />
          </IconButton>
        </Box>

        {expandedReplies[commentId] && comment.replies && (
          <Box ml={4} mt={1}>
        ...
        <Box display="flex" mt={1}>
          ...
          <Button
            ...
            variant="contained"
            disabled={!replyInputs[commentId]?.trim()}
            sx={{ ml: 1 }}
          >
            Reply
          </Button>
