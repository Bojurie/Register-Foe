...
const CommentSection = ({ topicId }) => {
  ...
  // Fetch comments when the component mounts or topicId changes
  useEffect(() => {
    const fetchComments = async () => {
      try {
        ...
        setComments(response.data.comments || []); // Set to empty array if undefined
  ...
  const handleAddComment = async () => {
    if (!newComment.trim()) return; // Validate comment input
    try {
  ...
  const handleReplySubmit = async (commentId) => {
    ...
    if (!text) return; // Validate reply input
    try {
      const response = await axiosInstance.post(
  ...
  // Handle reactions (like/dislike) on comments and replies
