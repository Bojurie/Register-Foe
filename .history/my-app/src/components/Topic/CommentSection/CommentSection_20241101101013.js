...
import { Box, Typography, TextField, IconButton, Alert, CircularProgress } from "@mui/material";
  ...
  const handleReaction = async (commentId, type, isReply = false) => {
    try {
      ...
      if (isReply) {
        ...
        if (parentComment) {
          const updatedReplies = parentComment.replies.map((reply) => (reply._id === commentId ? response.data.updatedReply : reply));
              ...
              <CommentBox key={comment._id}>
                ...
                <Box className="Comment-actions" display="flex" alignItems="center">
                  <IconButton onClick={() => handleReaction(comment._id, "like")}>
                    <FaThumbsUp /> {comment.likesCount || 0} {/* Corrected count */}
                  </IconButton>
                  <IconButton onClick={() => handleReaction(comment._id, "dislike")}>
                    <FaThumbsDown /> {comment.dislikesCount || 0} {/* Corrected count */}
                  </IconButton>
                  <IconButton
                    onClick={() =>
                ...
                {comment.replies.map((reply) => (
                  <CommentBox key={reply._id} mt={2} sx={{ paddingLeft: "16px" }}>
                    <Box display="flex" alignItems="center">
                      <UserImage
                    ...
                    <Box className="Reply-actions" display="flex" alignItems="center">
