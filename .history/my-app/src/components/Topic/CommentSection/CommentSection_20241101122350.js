...
const CommentSection = ({ topicId }) => {
  ...
  const handleReaction = async (id, type, isReply = false) => {
    try {
      const endpoint = isReply
        ? `/vote/vote/topics/${topicId}/comments/${id}/${type}`
        : `/vote/vote/topics/${topicId}/comments/${id}/${type}`;

      const response = await axiosInstance.post(endpoint);
      if (isReply) {
        const parentComment = comments.find((comment) =>
          comment.replies.some((reply) => reply._id === id)
        );
        if (parentComment) {
          const updatedReplies = parentComment.replies.map((reply) =>
            reply._id === id
              ? { ...reply, reactions: response.data.updatedReply.reactions }
              : reply
          );
