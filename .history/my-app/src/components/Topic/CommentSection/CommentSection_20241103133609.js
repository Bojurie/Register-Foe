  ...
  const handleVote = async (id, reaction) => {
    ...
    const updatedComments = comments.map((comment) => {
      if (comment._id === id) {
        ...
        updatedComment.reactions[reaction + "Count"]++;
        ...
        {loading ? (
          ...
          <>
            ...
            {comments.map((comment) => {
              const commentId = comment._id; // Accessing directly
              return (
                <Box
                  key={commentId}
                  mb={3}
                    ...
                    <Box ml={4} mt={1}>
                      {comment.replies &&
                        comment.replies.map((reply) => {
                          const replyId = reply._id; // Accessing directly
                          return (
                            <Box key={replyId} mb={1}>
                              <Typography variant="body2">
                                {reply.text} - By {reply.user.username}
                              </Typography>
                            </Box>
                          );
                        })}
