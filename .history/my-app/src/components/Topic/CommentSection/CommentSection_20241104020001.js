...
const CommentSection = ({ topicId }) => {
  ...
  const fetchComments = async () => {
    ...
    try {
      ...
      // Ensure likeCount and dislikeCount are initialized
  ...
  const handleVote = async (commentId, reaction) => {
    ...
    const updateCommentsList = (commentsList) =>
      commentsList.map((comment) => {
        const commentIdentifier = comment.id || comment._id?.$oid || null;
        if (commentIdentifier === commentId) {
          const updatedComment = { ...comment };
          updatedComment.reactions = {
            likeCount: updatedComment.reactions?.likeCount || 0,
            dislikeCount: updatedComment.reactions?.dislikeCount || 0,
          };

          // Increment the appropriate count based on the reaction
          if (reaction === "like") {
      ...
      <Box key={commentId} mb={3} borderBottom="1px solid #ddd" pb={2}>
        ...
        {expandedReplies[commentId] && (
          <Box
            ...
            overflow="auto" // Make replies section scrollable
