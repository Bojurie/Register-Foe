...
const handleCommentSubmit = async (commentId = null) => {
  // Check if we are submitting a reply or a new comment
  const endpoint = commentId
    ? `/vote/vote/topics/${topicId}/comments/${commentId}`
    : `/vote/vote/topics/${topicId}/comments`;

  if (!newComment) return; // Ensure there's text to submit
  ...
  const handleVote = async (commentId, action) => {
    try {
      await axiosInstance.post(`/vote/vote/comments/${commentId}/vote`, { action });
