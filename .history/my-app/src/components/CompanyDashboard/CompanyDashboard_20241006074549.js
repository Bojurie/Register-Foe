...
const useCompanyDashboardData = (user) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Assuming `getElectionById` should be called multiple times to get all elections.
      // Adjust as necessary if there is a specific list of election IDs you want to fetch.
      const electionIds = user.electionIds || []; // Assuming user object contains election IDs
      const electionsPromises = electionIds.map((id) => getElectionById(id));
      ...
      const [pastElections, savedElections, upcomingNews, users, ...otherData] =
        await Promise.all(fetchPromises);

      const upcomingElections = await Promise.all(electionsPromises);
