...
useEffect(() => {
  ...
  const fetchTopics = async () => {
    if (user && user.isCompany && user.companyCode) {
      console.log("User company code:", user.companyCode); // Debugging log
      try {
        const topicResponse = await fetchTopicsByCompanyCode(user.companyCode);
        if (topicResponse) {
         setTopics(topicResponse.data); // Make sure to use .data here
        }
        ...
        setIsLoading(false); // Moved inside the async function
      }
    } else {
      console.log("User or company code not available");
      setIsLoading(false); // Set to false when user or company code is not available
    }
  };
