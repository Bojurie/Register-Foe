...
const useCompanyDashboardData = (user, dependencies) => {
  const {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setState({ error: error.toString(), loading: false });
    }
  }, dependencies);
...
function CompanyDashboard() {
  ...
  const state = useCompanyDashboardData(user, [
    user,
    useAuth().fetchUserByCompanyCode,
    useAuth().getElections,
    useAuth().fetchTopicsByCompanyCode,
    useAuth().fetchAdminUsers,
    useAuth().getCompanyNews,
    useAuth().fetchSavedElections,
    useAuth().fetchPastElection,
  ]);
      ...
      >
        ...
        <DashboardButtons
          setIsElectionModalOpen={() =>
            toggleModal("isElectionModalOpen", true)
          }
          setIsTopicModalOpen={() => toggleModal("isTopicModalOpen", true)}
          setIsCompanyNewsModalOpen={() =>
            toggleModal("isCompanyNewsModalOpen", true)
          }
        />
        <DashboardSections
          ...
          return (
            isOpen && (
              <ModalComponent
                key={modalKey}
                isModalOpen={isOpen}
                setIsModalOpen={() => toggleModal(modalKey, false)}
                modalVariants={{
                  hidden: { opacity: 0 },
                  visible: { opacity: 1 },
                  exit: { opacity: 0 },
