...
function CompanyDashboard({ user }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const electionsResponse = await axios.get("/election/upcoming-elections");
  ...
  return (
    <div>
      <motion.div className="CompanyDashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.5 }}>
