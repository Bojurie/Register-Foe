...
useEffect(() => {
  const fetchTopics = async () => {
    if (topic && topic.isCompany && topic.companyCode) {
      console.log("Company code before fetch:", topic.companyCode); // Debugging
      await fetchTopicsByCompanyCode(topic.companyCode);
    } else {
  ...
  fetchTopics();
}, [topic, fetchTopicsByCompanyCode]);
