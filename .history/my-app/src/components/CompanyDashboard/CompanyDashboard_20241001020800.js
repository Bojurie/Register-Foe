  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const fetchPromises = [
        ...
        getElections(user.companyCode), // Fetching elections regardless of the user type to display them correctly
      ];
      ...
      const newState = {
        ...
        savedElections,
        upcomingNews,
        upcomingElections, // Ensure elections are fetched
        loading: false,
        error: null,
      };
...
function CompanyDashboard() {
  const { user } = useAuth(); // Using the user object from context
