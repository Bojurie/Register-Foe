...
function CompanyDashboard() {
  const { getUsersByCompanyCode, getElections, user } = useAuth();
  ...
  const modalVariants = {
   ...
   useEffect(() => {
     const fetchData = async () => {
       try {
         if (user && user.isCompany && user.companyCode) {
           const electionsResponse = ( `${BASE_URL}/election/upcoming-elections`)
           setUpcomingElections(electionsResponse);

           const candidatesResponse = await getUsersByCompanyCode(
             user.companyCode
           );
     ...
     fetchData();
   }, [user, getElections, getUsersByCompanyCode]);
