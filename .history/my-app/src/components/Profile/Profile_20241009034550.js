...
const Profile = ({ user }) => {
  ...
  const [hasLiked, setHasLiked] = useState(user?.likedBy?.includes(user._id)); // Check if the user has liked the profile
  const [likeMessage, setLikeMessage] = useState(""); // Display message if the user already liked
  ...
  const handleProfileLike = async () => {
    if (hasLiked || !user?._id) {
      setLikeMessage("You have already liked this profile."); // Display message
      return;
    }
    ...
    <div className="ProfileCard" onClick={handleProfileClick}>
      ...
      {isModalOpen && (
        <ProfileDetailsModal
          ...
          hasLiked={hasLiked}
          likeMessage={likeMessage} // Pass the message to the modal
        />
      )}
    </div>
  );
