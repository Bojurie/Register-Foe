...
const Profile = ({ user }) => {
  ...
  const [likeMessage, setLikeMessage] = useState(""); // To display a message if the user has already liked
  ...
  const handleProfileLike = async () => {
    if (hasLiked || !user?._id) {
      setLikeMessage("You have already liked this profile."); // Set the message
      return;
    }
    ...
    try {
      ...
      if (
        ...
        setLikeMessage("You have already liked this profile."); // Set the message if the user has already liked
      } else {
