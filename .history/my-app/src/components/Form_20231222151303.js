...
const Form = () => {
  ...
  const validateForm = () => {
    ...
    if (formData.isAdmin && !formData.companyCode.trim()) throw new Error("Company Code is required for admins");
    if (!/\S+@\S+\.\S+/.test(formData.email.trim())) throw new Error("Email is invalid");
  };
