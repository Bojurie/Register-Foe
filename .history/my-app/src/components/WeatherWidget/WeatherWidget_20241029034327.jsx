...
const WeatherWidget = () => {
  ...
  // Move city definition to be a state or constant
  const [city, setCity] = useState("");

  const randomCity = useCallback(() => {
    const cities = Object.keys(mockWeatherData.locations);
    const randomCity = cities[Math.floor(Math.random() * cities.length)];
    setCity(randomCity); // Set the city state here
  }, []);

  useEffect(() => {
    randomCity(); // Call randomCity when the component mounts
  }, [randomCity]);

  const fetchWeatherData = useCallback(() => {
    // Now you can access city here since it's defined in the state
