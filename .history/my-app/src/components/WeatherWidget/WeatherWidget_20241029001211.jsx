...
const WeatherWidget = () => {
  ...
  // Get a random city from the mock data
  const randomCity = useCallback(() => {
    const cities = Object.keys(mockWeatherData);
    return cities[Math.floor(Math.random() * cities.length)];
  }, []);
  
  const city = randomCity(); // Get a random city

  const fetchWeatherData = useCallback(() => {
    const weatherData = mockWeatherData[city];

    // Check if weatherData is defined and has a current property
    if (!weatherData || !weatherData.current) {
      setError("Weather data not available for this location.");
      setWeather(null); // Reset weather state if data is not available
      return;
    }
    ...
    >
      {error ? (
        ...
        >
          ...
          <motion.div className="Location">
            <p>
              {`${mockLocationData[city].city}, ${mockLocationData[city].state || ''}, ${mockLocationData[city].country}`}
            </p>
          </motion.div>
          <p>{currentDateTime.date}</p>
          <motion.div className="Temperature">
