...
const WeatherWidget = ({ locationData }) => {
  ...
  const fetchWeatherData = useCallback(async () => {
    ...
    // Use the OpenWeather API 3.0 to get weather data for the specified location
    try {
      const response = await fetch(
        `https://api.openweathermap.org/data/3.0/weather?city=${encodeURIComponent(
          city
...
useEffect(() => {
  if (weather) {
    ...
    // Check if weather and weather[0] exist before accessing
    const weatherArray = weather.weather || []; // Default to an empty array
    const weatherIcon = weatherArray.length > 0 ? weatherArray[0].icon : null; // Get the icon safely
