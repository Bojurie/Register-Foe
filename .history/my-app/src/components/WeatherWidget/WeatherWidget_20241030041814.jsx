...
const WeatherWidget = ({ locationData }) => {
  ...
  useEffect(() => {
    if (weather) {
      console.log("Weather Object: ", weather); // Log the weather object to check its structure
      const hour = new Date().getHours();
      const weatherArray = weather.weather || []; // Default to an empty array to avoid undefined access
      const weatherIcon = weatherArray.length > 0 ? weatherArray[0].icon : null; // Safely access the icon
