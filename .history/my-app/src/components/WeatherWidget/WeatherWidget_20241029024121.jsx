...
const WeatherWidget = () => {
  ...
  useEffect(() => {
    if (weather) {
      ...
      if (weather.weatherIcon === "01d") {
        updateTheme("clearSky");
      } else if (
        weather.weatherIcon.includes("09") ||
        weather.weatherIcon.includes("10")
      ) {
        updateTheme("rain");
      } else if (weather.weatherIcon.includes("13")) {
        updateTheme("snow");
      } else {
        updateTheme(hour < 6 || hour >= 18 ? "night" : "clearSky");
      }
    }
  }, [weather, updateTheme]);
