...
import React, { useState, useEffect, useCallback } from "react";
import axios from "axios"; // Retain axios for actual API calls
import { DateTime } from "luxon";
import {
  WiDaySunny,
  WiRain,
...
import { motion } from "framer-motion";
import "./WeatherWidget.css";
import { mockWeatherData, mockLocationData } from "./mocks/weatherMockAPI";

const WeatherWidget = ({ locationData = mockLocationData }) => {
  const [weather, setWeather] = useState(null);
  const [error, setError] = useState("");
  const [currentDateTime, setCurrentDateTime] = useState({});

  const fetchMockWeatherData = useCallback(() => {
    const { temp, weather: weatherInfo } = mockWeatherData.current;
    const { description, icon } = weatherInfo[0];
    ...
    updateCurrentDateTime(mockWeatherData.timezone);
  }, []);
  ...
  useEffect(() => {
    if (locationData?.latitude && locationData?.longitude) {
      fetchMockWeatherData(); // Call mock fetch function for testing
    }
  }, [locationData, fetchMockWeatherData]);
