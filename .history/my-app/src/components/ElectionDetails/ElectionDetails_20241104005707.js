...
const ElectionDetails = ({
  ...
  const { user } = useAuth();
  const [leadingCandidates, setLeadingCandidates] = useState([]);
  ...
  useEffect(() => {
    ...
    const currentLeaders = sortedCandidates.filter(
      (candidate) => candidate.votes === highestVoteCount
    );

    setLeadingCandidates(currentLeaders);
  }, [election]);
  ...
  return (
    <div className="ElectionDetails">
      ...
      <div className="election-poll">
        {isElectionEnded && leadingCandidates.length > 0 ? (
          <div className="election-winner">
            ...
            {leadingCandidates.length > 1 ? (
              ...
              <p>
                Winner: {leadingCandidates[0].user.firstName}{" "}
                {leadingCandidates[0].user.lastName}
              </p>
            )}
          ...
          <div className="leading-candidates">
