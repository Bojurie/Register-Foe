      ...
      <div className="election-poll">
        {isElectionEnded ? (
          tieCandidates.length > 1 ? (
            ...
            >
              <h3>
                <FontAwesomeIcon icon={faHandshake} className="tie-icon" /> Tie
                for Winner:
              </h3>
              <ul>
              ...
              <p>
                Each with {calculatePercentage(tieCandidates[0].votes)}% of the
                vote
              </p>
            </motion.div>
          ) : currentLeader ? (
          ...
          <motion.div
            className="tie-indication"
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
          >
            <h3>Currently Tied for Leading:</h3>
            <ul>
              {tieCandidates.map((candidate) => (
                <li key={candidate._id}>
                  {candidate.user.firstName} {candidate.user.lastName}
