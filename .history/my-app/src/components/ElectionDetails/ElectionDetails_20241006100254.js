...
const ElectionDetails = ({ election }) => {
  ...
  const calculateVotePercentage = (profile) => {
    const totalVotes = userProfiles.reduce(
      (sum, user) => sum + (user.votesCount || 0),
      0
    );
    return totalVotes > 0 ? ((profile.votesCount || 0) / totalVotes) * 100 : 0;
  };
    ...
    if (userProfiles.length > 0) {
      return userProfiles.map((profile) => (
        <Profile
          key={profile.id}
          user={profile}
          votePercentage={calculateVotePercentage(profile)}
        />
      ));
    }
  ...
  return (
    <div className="ElectionDetails">
      ...
      <div className="candidates-section">
        <h3>Candidates</h3>
        <div className="candidates-grid">{renderCandidateProfiles()}</div>
      </div>
