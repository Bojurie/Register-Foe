...
import { useTheme } from "../WeatherWidget/StyledComponents";
...
const DashboardHeader = ({ user }) => {
  ...
  const fetchLocationDetails = async (latitude, longitude) => {
    ...
    try {
      ...
      if (results?.length > 0) {
        ...
        setCurrentTime(currentDateTime.getHours());
      } else {
  ...
  useEffect(() => {
    // Update the theme based on weather icon and current hour
    if (weatherIcon) {
      updateTheme(weatherIcon, currentTime);
    }
  }, [weatherIcon, currentTime, updateTheme]);
