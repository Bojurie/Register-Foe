...
router.get("/election/:electionId", verifyToken, async (req, res) => {
  ...
  try {
    // Fetch the election by its ID and make sure it matches the company code for authorization
    const election = await Election.findOne({
      _id: electionId,
    }).populate("candidates.user", "firstName lastName votes"); // Populate only needed fields

    if (!election) {
      return res.status(404).json({
        message:
          "Election not found or you do not have permission to access it.",
      });
    }

    // Calculate total votes only if candidates exist
    let totalVotes = 0;
    if (election.candidates && election.candidates.length > 0) {
      totalVotes = election.candidates.reduce(
        (acc, candidate) => acc + (candidate.votes || 0),
        0
      );
    }

    // Append vote percentages to each candidate
