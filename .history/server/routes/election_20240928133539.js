...
router.get("/election/:id", verifyToken, async (req, res) => {
  const { electionId } = req.params;
  // const userCompanyCode = req.user.companyCode;

  try {
    // Find the election by ID and ensure the company code matches
    ...
    if (!election) {
      return res
        .status(404)
        .json({ message: "Election not found or access denied" });
    }

    // Process the candidates and calculate vote percentages
