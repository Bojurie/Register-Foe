...
router.post("/elections/:electionId/vote", verifyToken, async (req, res) => {
  ...
  console.log("Received data:", { userId, electionId, candidateId });
  ...
  try {
    ...
    // Check if voting is allowed based on the current date
    const currentDate = new Date();
    if (currentDate < election.dateStart || currentDate > election.dateEnd) {
      console.log(`Voting not allowed. Current date: ${currentDate}`);
      return res
        .status(400)
    ...
    if (user.votedElections.includes(electionId)) {
      console.log(
        `User ${user.firstName} ${user.lastName} has already voted in election ${electionId}`
      );
      return res
        .status(400)
    ...
    console.log(
      `Candidate found: ${candidate.user.firstName} ${candidate.user.lastName}`
    );
    ...
    console.log(
      `Candidate ${candidate.user.firstName} ${candidate.user.lastName} now has ${candidate.votes} votes`
    );
    ...
    await user.save();
    console.log(
      `User ${user.firstName} ${user.lastName} successfully voted in election ${election.title}`
    );

    // Respond with the updated vote count for the candidate
    res.status(200).json({
