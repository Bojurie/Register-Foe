...
router.patch("/news/markAsRead/:newsId", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Check if the user has already read the news, if not, mark it as read
    if (!news.readBy.includes(userId)) {
      news.readBy.push(userId); // Add userId to the 'readBy' array
      await news.save(); // Save the changes
