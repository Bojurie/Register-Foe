...
outer.post("/elections/:electionId/vote", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const electionsWithCandidates = elections.map((election) => {
      ...
      const leadingCandidate = candidatesWithPercentages.reduce(
        (prev, current) => {
          return (prev.voteCount || 0) > (current.voteCount || 0)
            ? prev
            : current;
        },
        {}
      ...
      return {
        ...
        leadingCandidateVoteCount: leadingCandidate.voteCount || 0, // Add leading candidate vote count
      };
    });
