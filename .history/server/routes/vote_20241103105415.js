...
router.post(
  ...
  async (req, res) => {
    const { action, text } = req.body; // Extract action and text from the body
    const { topicId, commentId } = req.params; // Get IDs from URL params
    const userId = req.user.id; // Assuming userId is extracted from the request context
    ...
    if (action && !["like", "dislike"].includes(action)) {
      console.error("Invalid action received:", action); // Log the invalid action
      return res
        .status(400)
        .json({ message: "Invalid action. Use 'like' or 'dislike'." });
    }
    ...
    try {
      ...
      // Case 1: Adding a new comment if commentId is not provided
      if (!commentId && text) {
        const newComment = {
          user: userId,
          text: text.trim(),
      ...
      if (action) {
        ...
        if (comment.reactions[`${oppositeAction}s`].includes(userId)) {
        ...
        if (!comment.reactions[`${action}s`].includes(userId)) {
          comment.reactions[`${action}s`].push(userId);
        }

        // Update counts
        comment.reactions[`${action}Count`] =
          comment.reactions[`${action}s`].length;
        comment.reactions[`${oppositeAction}Count`] =
          comment.reactions[`${oppositeAction}s`].length;
      ...
      // Case 3: Adding a reply to a comment
      if (text) {
        const newReply = {
          user: userId,
          text: text.trim(),
