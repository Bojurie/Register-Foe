...
router.get("/voted/:userId", verifyToken, async (req, res) => {
  const userId = req.user._id; // Ensure that userId is being retrieved correctly
  try {
    if (!userId) {
    ...
    console.error("Error fetching voted past elections:", error); // Log full error details
