...
router.get("/byCompanyCode/:companyCode", verifyToken, async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[INFO] Fetched and processed ${newsWithReadStatus.length} news items for userId: ${userId}`
    );
...
router.patch("/markAsRead/:newsId", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Add userId to readBy array only if not already included
    ...
    // Return the updated status of the news item
