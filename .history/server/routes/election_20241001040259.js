...
router.post("/create-election", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Check if all candidate users exist
    const candidateDocs = await User.find({ _id: { $in: validCandidateIds } });
    if (candidateDocs.length !== validCandidateIds.length) {
      return res
        .status(400)
        .json({ error: "Some candidate IDs do not match any existing users." });
    }

    // Create the election document
    const newElection = new Election({
      ...electionData,
      candidates: [], // We'll add candidate references later
    });

    await newElection.save();

    // Create candidate documents and associate with the election
    const candidatePromises = candidateDocs.map((userDoc) => {
      const newCandidate = new Candidate({
        user: userDoc._id,
        election: newElection._id,
        partyAffiliation: userDoc.partyAffiliation || "Independent",
      });
    ...
    // Update election with candidate references
    newElection.candidates = savedCandidates.map((candidate) => candidate._id);
    await newElection.save();
