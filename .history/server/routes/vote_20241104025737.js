...
router.get("/vote/topics/:topicId/comments/:commentId/reactions", verifyToken, async (req, res) => {
  const { topicId, commentId } = req.params;

  if (!topicId || !commentId) {
    return res.status(400).json({ message: "Topic ID and Comment ID are required." });
  }

  try {
    const topic = await Topic.findById(topicId).populate("comments.reactions.likes", "username profilePicture")
