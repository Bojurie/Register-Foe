...
router.get("/elections/past/:companyCode", verifyToken, async (req, res) => {
  try {
    ...
    // Query to find past elections
    const electionQuery = {
      companyCode: requestedCompanyCode,
      endDate: { $lt: currentDate },
    };

    // Limit results for non-admin users who aren't part of the company
    ...
    // Fetch elections, populate candidate user details
    const elections = await Election.find(electionQuery)
      .populate({
        path: "candidates.user",
        select: "firstName lastName", // Fetch relevant user details
      })
    ...
    // Process elections to calculate total votes and find the winning candidate
    ...
    // Return the elections with additional details
