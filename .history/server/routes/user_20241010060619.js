...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Validate the companyCode before proceeding
      ...
      // Save the user to the database (password hashing is done in schema's pre-save)
      await user.save();
      ...
      // Return success response with token and user details (excluding sensitive info)
      res.status(201).json({
        user: {
          _id: user._id,
          email: user.email,
