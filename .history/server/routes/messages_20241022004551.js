...
router.post("/messages", verifyToken, async (req, res) => {
  ...
  if (!conversationId || !content || !recipientId) {
    return res.status(400).json({ message: "Conversation ID, content, and recipient ID are required." });
  }
  ...
  try {
    ...
    await Conversation.findByIdAndUpdate(
      conversationId,
      { lastMessage: content, lastUpdated: Date.now() },
      { session }
    );
...
router.get("/messages/:userId", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const formattedMessages = messages.map((message) => {
      ...
      return {
        ...message.toObject(),
        senderName, // Include formatted sender name
      };
    });
