...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Fetch users with the requested company code
      const users = await User.find({
        companyCode: requestedCompanyCode,
      }).lean();

      // Log the users fetched from the database for debugging
      console.log("Users found:", users);

      // If no users are found, return a response indicating no users found
      if (!users.length) {
      ...
      // Add virtual fields or additional user-specific details if needed
      ...
      // Log the users with virtual fields for debugging
      console.log("Users with virtual fields:", usersWithVirtuals);

      // Return the total number of users along with the users data
      return res.status(200).json({
        totalUsers: users.length, // Total number of users with the company code
