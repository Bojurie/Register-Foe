...
router.get("/elections/candidates/:electionId", verifyToken, async (req, res) => {
  try {
    ...
    // Find the election by ID and populate candidate data
    const election = await Election.findById(electionId).populate({
      path: "candidates.candidateId",
      model: "User",
    ...
    // Ensure the populated candidates have the required data
    const candidates = election.candidates
      .filter((candidate) => candidate.candidateId) // Only keep candidates with valid `candidateId`
      .map((candidate) => ({
    ...
    if (candidates.length === 0) {
      return res
        .status(404)
        .json({ error: "No valid candidates found for this election." });
    }
