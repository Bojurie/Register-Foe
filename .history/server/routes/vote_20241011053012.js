...
router.get("/topics/:id/:action", verifyToken, async (req, res) => {
  ...
  // Validate action type (either 'likes' or 'dislikes')
  ...
  try {
    // Populate 'likes' or 'dislikes' based on the action
    const topic = await Topic.findById(id).populate(action, "_id username");
    if (!topic) {
      return res.status(404).json({ message: "Topic not found" });
    }

    // Calculate count dynamically based on the action
    const count = topic[action].length;

    return res.status(200).json({
      message: `Successfully fetched ${action} count.`,
      [`${action}Count`]: count,
      [action]: topic[action], // Return the list of users who liked/disliked
    });
  } catch (error) {
