...
router.post("/create-election", authenticateJWT, async (req, res) => {
  try {
    const {
      ...
      electionEndDate,
      electionDesc,
      candidates, // This should be an array of user IDs
    } = req.body;

    // Check if the authenticated user is a company or an admin
    ...
    // Fetch companyCode of the user creating the election
    const userCreatingElection = await User.findById(req.user._id);
    if (!userCreatingElection) {
      return res.status(404).json({ error: "User not found" });
    }

    // Validate that each candidate has the same companyCode
    ...
    const newElection = new Election({
      ...
      electionStartDate,
      electionEndDate,
      createdBy: req.user._id, // Link the election to the creating entity
      electionDesc,
