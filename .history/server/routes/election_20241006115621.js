...
// Server-side (Backend): Updated /elections/candidates/:electionId Route
router.get("/elections/candidates/:electionId", verifyToken, async (req, res) => {
  try {
    const { electionId } = req.params;

    // Validate if electionId is a valid ObjectId
    if (!mongoose.Types.ObjectId.isValid(electionId)) {
      return res.status(400).json({ error: "Invalid Election ID format" });
    }

    // Find the election and populate the candidates' details
    const election = await Election.findById(electionId).populate({
      path: "candidates.candidateId",
      model: "User",
      select: "firstName lastName age sex userProfileImage votesCount role userProfileDetail",
    });
    ...
    // Ensure that candidates have valid user data
    const candidates = election.candidates
      .filter((candidate) => candidate.candidateId) // Filter out invalid or null candidateId references
      .map((candidate) => ({
        id: candidate.candidateId._id,
        firstName: candidate.candidateId.firstName,
        lastName: candidate.candidateId.lastName,
        sex: candidate.candidateId.sex,
        role: candidate.candidateId.role,
        userProfileDetail: candidate.candidateId.userProfileDetail,
        age: candidate.candidateId.age,
        userProfileImage: candidate.candidateId.userProfileImage,
        votesCount: candidate.voteCount,
        votePercentage: candidate.votePercentage || "0.00", // Assuming `votePercentage` is part of the schema
      }));
    ...
    // Send the election data including valid candidates
