...
router.post(
  ...
  async (req, res) => {
    ...
    console.log("Received topicId:", req.params.topicId); // Log the topicId
    console.log("Received commentId:", req.params.commentId); // Log the commentId
    ...
    try {
      const topic = await Topic.findById(topicId);
      if (!topic) {
        console.log("Topic not found."); // Log if topic is not found
        return res.status(404).json({ message: "Topic not found." });
      }

      // Check if a comment ID is provided (for adding replies)
      if (commentId) {
        const comment = topic.comments.id(commentId);
        if (!comment) {
          console.log("Comment not found."); // Log if comment is not found
          return res.status(404).json({ message: "Comment not found." });
        }
        ...
        comment.replies.push(reply); // Add reply to the comment
        await topic.save(); // Save the topic with the new reply
        ...
        // Creating a new comment if no comment ID is provided
        const newComment = {
          user: req.user._id,
          text: text.trim(),
          createdAt: new Date(),
        ...
        topic.comments.push(newComment); // Add new comment to the topic
        await topic.save(); // Save the topic with the new comment
