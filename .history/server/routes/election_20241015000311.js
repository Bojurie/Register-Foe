...
router.get("/elections/past", verifyToken, async (req, res) => {
  try {
    const { isCompany, role, _id: userId } = req.user;
    const currentDate = new Date();

    let elections;

    if (isCompany || role === "Admin") {
      // Admins or company users can access all past elections
      ...
      // Regular users can only access elections where they were either voters or candidates
...
router.post("/elections/:electionId/vote", verifyToken, async (req, res) => {
  ...
  try {
    ...
    console.log("Election Start Date:", startDate);
    console.log("Election End Date:", endDate);

    if (currentDate < startDate || currentDate > endDate) {
      console.log("Voting is not allowed at this time.");
      return res
        .status(400)
...
router.get("/elections/:companyCode", verifyToken, async (req, res) => {
  ...
  try {
    // Check if the user and company share the same companyCode or if the user is an Admin
    ...
    // Fetch elections for the shared companyCode and that are upcoming
    const elections = await Election.find({
      companyCode: requestedCompanyCode,
      endDate: { $gt: currentDate }, // Retrieve only upcoming elections
    })
      .populate({
        path: "candidates.candidateId",
        model: "User",
        select: "firstName lastName", // Select candidate details for the election
      })
      .lean();
    ...
    if (!elections.length) {
      return res
        .status(404)
        .json({ error: "No active elections found for this company code." });
    }

    // Calculate total votes and vote percentage for each candidate
    ...
    // Return the upcoming elections with detailed candidate information
