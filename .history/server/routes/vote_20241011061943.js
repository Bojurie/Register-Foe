...
router.get("/topics/:id/:action", verifyToken, async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      [`${action}Count`]: count,
      [`${action}`]: topic.reactions[action], // Return the actual likes/dislikes data
    });
  } catch (error) {
...
// NEW POST LIKE AND DISLIKE COMMENT
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      let updatedComment;
      if (action === "like") {
        updatedComment = await topic.likeComment(commentId, userId);
      } else {
        updatedComment = await topic.dislikeComment(commentId, userId);
      }
...
router.get("/topics/:topicId/comments", verifyToken, async (req, res) => {
  ...
  try {
    const topic = await Topic.findById(topicId)
      .populate({
        path: "comments.user", // Populate user data for comments
        select: "username firstName lastName",
      })
      .populate({
        path: "comments.replies.user", // Populate user data for replies
        select: "username firstName lastName",
      });
