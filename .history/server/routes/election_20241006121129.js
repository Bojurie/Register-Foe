...
router.get("/elections/candidates/:electionId", verifyToken, async (req, res) => {
  try {
    ...
    // Fetch the election and populate candidate user data
    const election = await Election.findById(electionId).populate({
      path: "candidates",
      populate: {
        path: "user",
    ...
    console.log("Election found, candidates length:", election.candidates?.length);
    ...
    // Extract candidate details with proper checks to ensure consistency
    const candidates = election.candidates
      .map((candidate) => {
        if (candidate.user) {
          return {
          ...
          console.log("Invalid candidate data found for election:", candidate);
          return null;
        }
      })
      .filter((candidate) => candidate !== null); // Filter out invalid candidates

    // Return the election data along with candidates for consistent display in the frontend
    res.status(200).json({ election: { ...election.toObject(), candidates } });
  } catch (error) {
    console.error("Error fetching candidates:", error);
    res.status(500).json({ error: "Failed to fetch candidates." });
  }
