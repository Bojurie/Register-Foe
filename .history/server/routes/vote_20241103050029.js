...
try {
  ...
  // Convert `onlyCount` to a boolean to handle cases like "false"
  const shouldOnlyReturnCount = onlyCount === "true";

  if (shouldOnlyReturnCount) {
    return res.status(200).json({
  ...
  // Fetch full comments data with populated fields if `onlyCount` is not true
