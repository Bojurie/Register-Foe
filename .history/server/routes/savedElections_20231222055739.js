...
// Save Election
router.post("/save-elections", authenticateJWT, async (req, res) => {
  try {
    if (!req.user) {
    ...
    res.status(500).json({ 
      ...
      message: error.message, // Log the specific error message
      stack: error.stack // Optional: Log the error stack for detailed debugging (not recommended for production)
    })
  };
}
