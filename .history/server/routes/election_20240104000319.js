...
// Define the endpoint in your API
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if ((!companyCode, !electionId)) {
        return res.status(400).json({ message: "no electionId or companyCode is required" });
      }
      const electionData = await getElectionsByIds(companyCode, electionId);
