...
router.get("/elections/:companyCode", verifyToken, async (req, res) => {
  ...
  try {
    // Get the current date to compare with the election's end date
    const currentDate = new Date();

    // Find elections that are still ongoing (end date is in the future)
    const elections = await Election.find({
      companyCode,
      dateEnd: { $gt: currentDate }, // Filter to get only ongoing elections
    })
      .populate({
        path: "candidates.candidateId",
        model: "User",
    ...
    // Map over elections to calculate vote percentage
