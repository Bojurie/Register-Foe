...
router.post("/vote/topics/:topicId/:action", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Handle topic like/dislike
    if (action === "like") {
      // Like the topic
      if (!topic.reactions.likes.includes(userId)) {
        topic.reactions.likes.push(userId);
        topic.reactions.likeCount = topic.reactions.likes.length;

        // Remove user from dislikes if they had previously disliked the topic
        topic.reactions.dislikes = topic.reactions.dislikes.filter(
          (id) => !id.equals(userId)
        );
        topic.reactions.dislikeCount = topic.reactions.dislikes.length;

        console.log(`User ${userId} liked topic ID: ${topicId}`);
      } else {
        console.log(`User ${userId} has already liked topic ID: ${topicId}`);
      }
    } else if (action === "dislike") {
      // Dislike the topic
      if (!topic.reactions.dislikes.includes(userId)) {
        topic.reactions.dislikes.push(userId);
        topic.reactions.dislikeCount = topic.reactions.dislikes.length;

        // Remove user from likes if they had previously liked the topic
        topic.reactions.likes = topic.reactions.likes.filter(
          (id) => !id.equals(userId)
        );
        topic.reactions.likeCount = topic.reactions.likes.length;

        console.log(`User ${userId} disliked topic ID: ${topicId}`);
      } else {
        console.log(`User ${userId} has already disliked topic ID: ${topicId}`);
      }
    }
...
router.post(
  ...
  async (req, res) => {
    ...
    console.log(
      `Received ${action} request for topic ID: ${topicId} from user ID: ${userId}`
    );
    ...
    try {
      ...
      if (action === "like") {
        if (!topic.reactions.likes.includes(userId)) {
          ...
          const previousDislike = topic.reactions.dislikes.includes(userId);
          if (previousDislike) {
            topic.reactions.dislikes = topic.reactions.dislikes.filter(
              (id) => !id.equals(userId)
            );
        ...
        if (!topic.reactions.dislikes.includes(userId)) {
          ...
          const previousLike = topic.reactions.likes.includes(userId);
          if (previousLike) {
            topic.reactions.likes = topic.reactions.likes.filter(
              (id) => !id.equals(userId)
            );
          ...
          console.log(
            `User ${userId} has already disliked topic ID: ${topicId}`
          );
        }
      }
      ...
      console.error(
        `Error processing ${action} on topic ID: ${topicId}:`,
        error.message
      );
      return res
        .status(500)
...
// GET COMMENTS FOR A TOPIC MAIN GET ROUTE
