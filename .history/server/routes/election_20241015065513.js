...
router.get("/elections", verifyToken, async (req, res) => {
  try {
    ...
    const { companyCode: userCompanyCode, role } = req.user; // Extract user info from the verified token

    // Check if the user has a valid companyCode
    if (!userCompanyCode) {
      return res
        .status(400)
        .json({ message: "Invalid user. Company code is required." });
    }

    // Fetch all elections where the companyCode matches the user's and the election is still active
    ...
    if (!elections.length) {
      return res
        .status(404)
        .json({ message: "No active elections found for this company." });
    }

    // Format the elections and candidates
