...
router.get("/messages/inbox", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Modify the response to include companyName conditionally
    const formattedMessages = messages.map((message) => {
      const sender = message.sender;

      // Check if the sender exists and conditionally format the response
      if (!sender) {
        return {
          ...message.toObject(),
          sender: "Unknown Sender", // Set a fallback for missing sender
        };
      }
