...
router.get("/elections/:companyCode", verifyToken, async (req, res) => {
  ...
  // Validate the presence of companyCode in request params
  if (!requestedCompanyCode) {
    return res.status(400).json({ error: "Company code is required." });
  }

  try {
    // Ensure only users from the same company or admins can access the data
    if (requestedCompanyCode !== userCompanyCode && role !== "Admin") {
      return res
        .status(403)
    ...
    // Fetch all elections for the company
    ...
    const currentElectionsCount = await Election.countDocuments({
      companyCode: requestedCompanyCode,
      endDate: { $gte: currentDate }, // Ongoing elections have an endDate greater than or equal to currentDate
    });

    const elections = await Election.find({
      companyCode: requestedCompanyCode,
      endDate: { $gte: currentDate }, // Include only ongoing elections
    })
      .populate({
