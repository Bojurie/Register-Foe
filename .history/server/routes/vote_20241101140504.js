...
// POST TOPIC
router.post("/topics/:topicId?/:action?", verifyToken, async (req, res) => {
...
router.get("/vote/topics/:topicId/comments", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Return only the count if `onlyCount` query parameter is true
    if (onlyCount === "true") {
      return res.status(200).json({
    ...
    // Populate and format comments and replies when full data is requested
    const fullTopic = await topic
      .populate({
        path: "comments.user",
