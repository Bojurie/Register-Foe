...
router.get("/topics/:id/comments/count", async (req, res) => {
  const { id } = req.params;

  try {
    const topic = await Topic.findById(id).populate("comments"); // Ensure comments are populated if needed
    if (!topic) {
      return res.status(404).json({ message: "Topic not found" });
    }

    return res.status(200).json({
      count: topic.comments.length, // Assuming comments are stored as an array in the topic
    });
  } catch (error) {
