...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Fetch both current and total topics concurrently
      const [currentTopics, totalTopics] = await Promise.all([
        // Current topics: topics that are ongoing
        Topic.find({
        ...
        // Total topics: all topics regardless of date
        Topic.find({
          companyCode: requestedCompanyCode,
        }).select(
          "title dateStart dateEnd description likeCount dislikeCount comments"
      ...
      return res.status(200).json({
        currentTopics,
        totalTopicCount: totalTopics.length,
      });
    } catch (error) {
