...
router.get(
  ...
  verifyToken, // Middleware to ensure the user is authenticated
  async (req, res) => {
    const { companyCode: requestedCompanyCode } = req.params; // The requested company code from the params
    const { companyCode: userCompanyCode, role, isCompany } = req.user; // The user's company code, role, and status

    // Validate the presence of the company code in the request params
    if (!requestedCompanyCode) {
      return res.status(400).json({ error: "Company code is required" });
    }

    try {
      // Ensure that users can only access users within the same company or that admins can access any company
      if (
      ...
      // Fetch all users with the requested company code
      ...
      console.log(
        `Total users for companyCode ${requestedCompanyCode}: ${users.length}`
      );

      // If users are found, return them with calculated virtual fields
      if (users.length > 0) {
        ...
        return res.status(200).json({
          totalUsers: users.length, // Return the total number of users
          users: usersWithVirtuals,
        });
      }

      // If no users are found, return an empty array with totalUsers as 0
      return res.status(200).json({
        totalUsers: 0,
        users: [],
      ...
      // Log the error for debugging purposes and return a server error
      console.error(
        `Error retrieving users for companyCode ${requestedCompanyCode}:`,
        error
      );
