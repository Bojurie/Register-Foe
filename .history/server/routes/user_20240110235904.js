...
router.get("/users/validate-token", verifyToken, (req, res) => {
  try {
    console.log("Token validation passed for user:", req.user.id);
    res.status(200).json({ valid: true });
  } catch (error) {
    console.error("Error in /users/validate-token route:", error);
    res.status(500).json({ error: "Server error in token validation" });
  }
});
