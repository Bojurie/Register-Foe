...
router.get("/elections", verifyToken, async (req, res) => {
  try {
    ...
    // Fetch upcoming elections that match the user's company code
    const elections = await Election.find({
      companyCode: userCompanyCode, // Only elections with the same company code
      endDate: { $gt: currentDate }, // Only upcoming elections
    }).populate({
      path: "candidates",
      populate: {
        path: "user",
    ...
    // Handle case where no elections are found
    ...
    // Send the formatted elections back as response
