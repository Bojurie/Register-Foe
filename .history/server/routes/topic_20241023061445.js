...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Ensure the user has access to this company's topics or is an Admin
      ...
      console.log("Fetching topics for company code:", requestedCompanyCode);

      const currentDate = new Date();

      // Find active topics for the company
      ...
      ).sort({ dateStart: -1 }); // Sorting by the most recent topics

      // If no topics found, return 200 with an empty array (better UX than 404)
      if (!topics.length) {
