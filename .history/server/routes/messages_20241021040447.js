...
router.post("/messages", verifyToken, async (req, res) => {
  ...
  if (!conversationId || !content || !recipientId) {
    return res.status(400).json({
      message: "Conversation ID, content, and recipient ID are required.",
    });
  }
...
router.get("/messages/:conversationId", verifyToken, async (req, res) => {
  ...
  if (!ObjectId.isValid(conversationId)) {
    console.log("Invalid conversation ID:", conversationId); // Log invalid ID case
  ...
  try {
    ...
    const formattedMessages = messages.map((message) => {
      ...
      if (!sender) {
        return {
          ...message.toObject(),
          senderName: "Unknown Sender", // Assign unknown sender if null
        };
      }
      ...
      return {
        ...message.toObject(),
        senderName, // Set sender name for company or individual
      };
    });
