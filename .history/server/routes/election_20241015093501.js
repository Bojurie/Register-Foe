...
router.get("/elections", verifyToken, async (req, res) => {
  try {
    const { companyCode: userCompanyCode, role } = req.user; // Extract user company code and role
    const currentDate = new Date();

    // Find elections for the user's company that are upcoming
    const elections = await Election.find({
      companyCode: userCompanyCode, // Only fetch elections for the same company
      endDate: { $gt: currentDate }, // Only upcoming elections (endDate in the future)
    }).populate({
      path: "candidates",
      populate: {
        path: "user",
    ...
    // Check if any elections were found
    ...
    // Format the elections and candidates for the response
    ...
    // Send formatted elections back as response
