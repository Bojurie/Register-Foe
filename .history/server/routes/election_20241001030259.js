...
router.get("/elections/:companyCode", verifyToken, async (req, res) => {
  ...
  try {
    // Fetch all elections that belong to the company code
    const elections = await Election.find({ companyCode })
      .populate({
        path: "candidates",
        populate: {
          path: "user",
          model: "User", // Ensure User model is also properly imported and referenced
    ...
    if (!elections || elections.length === 0) {
      return res.status(404).json({
        error: "No elections found for the provided company code.",
      });
    }

    // Calculate vote percentages for each candidate in each election
    ...
    res.status(200).json({ elections: electionsWithVotePercentages });
  } catch (error) {
    console.error("Error fetching elections by company code:", {
      error: error.message,
      companyCode,
    });
    res.status(500).json({
      message: "An error occurred while fetching the elections.",
      error: error.message,
    });
  }
});
