...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Convert the newComment to a plain object to avoid Mongoose document issues
      return res.status(201).json({
        message: "Comment added successfully.",
        comment: {
          ...newComment,
          _id: topic.comments[topic.comments.length - 1]._id, // Include the generated comment ID
        },
      });
    } catch (error) {
      console.error(`Error adding comment to topic ID ${topicId}:`, error);
...
router.get("/vote/topics/:topicId/comments", verifyToken, async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      commentsCount: sortedComments.length, // Updated to use sortedComments for accuracy
    });
  } catch (error) {
    console.error("Error fetching comments:", error); // Log the full error object for more details
    return res
      .status(500)
      .json({ message: "Server error.", error: error.message });
  }
