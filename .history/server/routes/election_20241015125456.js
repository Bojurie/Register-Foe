...
router.get("/elections/past/:companyCode", verifyToken, async (req, res) => {
  try {
    ...
    // Ensure the requested company code is provided
    if (!requestedCompanyCode) {
      return res.status(400).json({ message: "Company code is required." });
    }

    // Check if the user has permission to view elections from the requested company
    ...
    // Define the query for fetching past elections
    const electionQuery = {
      companyCode: requestedCompanyCode,
      endDate: { $lt: currentDate },
    };

    // For non-admin and non-company users, restrict based on candidates or voters
    ...
    // Fetch past elections with candidates populated
    const elections = await Election.find(electionQuery)
      .populate({
        path: "candidates.user",
        select: "firstName lastName", // Populate relevant user details
      })
      .lean(); // Improve performance with lean query

    // If no elections are found, return a 200 response with an empty array
    if (!elections.length) {
    ...
    // Add extra details to each election, such as total votes and winning candidate
    ...
    // Return the processed elections with the extra details
