...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Fetch users and include virtuals for birthday, new hire, and anniversary
      const users = await User.find({ companyCode }).lean(); // `lean()` makes it plain JSON
      ...
      // Add virtual fields to each user
      const usersWithVirtuals = users.map((user) => {
        const birthdayCountdown = user.birthdayCountdown
          ? `${user.birthdayCountdown} days until birthday`
          : null;
        const isNewHire = user.isNewHire ? "New Hire" : null;
        const anniversaryCountdown = user.anniversaryCountdown
      ...
      res.status(200).json(usersWithVirtuals);
    } catch (error) {
