      ...
      if (commentId && action) {
        ...
        if (action === "like" || action === "dislike") {
          ...
          if (userAlreadyReacted) {
            return res
              .status(400)
              .json({ message: `You can only ${action} a comment once.` });
          }
...
router.get("/vote/topics/:topicId/comments", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Log the topic including comments for debugging purposes
    console.log("Comments fetched from the database:", topic);
