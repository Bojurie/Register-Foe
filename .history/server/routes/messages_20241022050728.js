...
// GET ALL MESSAGE FOR A SPECFIC MESSAGE.
router.get("/messages", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const conversations = await Conversation.find({
      ...
      path: "participants",
      select: "_id firstName lastName username isCompany companyName",
    });
    ...
    if (!messages.length) {
      console.log("No messages found for user conversations:", userId);
      return res
        .status(404)
        .json({ error: "No messages found for the user's conversations." });
    }

    // Send the conversations and messages as part of the response
    ...
    console.error(
      "Error fetching conversations or messages for user:",
      userId,
      error
    );
    return res
...
router.get("/messages/:conversationId", verifyToken, async (req, res) => {
  ...
  console.log("Received conversation ID:", conversationId); // Log the conversation ID
  ...
  try {
    ...
    if (!isParticipant) {
      ...
      return res
        .status(403)
        .json({ error: "You are not authorized to view this conversation." });
    }
    ...
    console.log("Messages retrieved successfully:", messages); // Log the messages returned
