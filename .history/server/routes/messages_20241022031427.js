...
router.get("/messages/:conversationId", verifyToken, async (req, res) => {
  ...
  // Log the received conversationId and userId
  console.log("Received conversation ID:", conversationId, "for user:", userId);
  ...
  try {
    ...
    // If conversation not found or user is not a participant, return error
    if (!conversation) {
    ...
    // Fetch messages for the conversation if the user is a participant
