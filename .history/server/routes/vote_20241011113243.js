...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Find the comment within the topic
      const comment = topic.comments.id(commentId);
      if (!comment) {
        return res.status(404).json({ message: "Comment not found" });
      }
      ...
      if (action === "like") {
        if (hasDisliked) {
          // If user previously disliked, remove from dislikes
      ...
      if (action === "dislike") {
        if (hasLiked) {
          // If user previously liked, remove from likes
          comment.likes.pull(userId);
        }
        if (!hasDisliked) {
      ...
      // Save the updated topic with the modified comment reactions
      await topic.save();
