...
router.get("/elections/:companyCode", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const currentElections = await Election.find({
      companyCode: requestedCompanyCode,
      endDate: { $gte: currentDate }, // Current elections
    })
      .populate({
        path: "candidates",
        populate: {
    ...
    // Log the fetched current elections
    console.log(
      "Fetched Current Elections:",
      JSON.stringify(currentElections, null, 2)
    );

    const currentElectionsWithCandidates = currentElections.map((election) => {
      const totalVotes = election.candidates.reduce(
    ...
    // Log the fetched past elections
    console.log(
      "Fetched Past Elections:",
      JSON.stringify(pastElections, null, 2)
    );

    const pastElectionsWithCandidates = pastElections.map((election) => {
      const totalVotes = election.candidates.reduce(
        (acc, candidate) => acc + (candidate.votes || 0),
        0
      );
    ...
    // Fetch total number of elections for the company (past + current)
    const totalElectionsCount = await Election.countDocuments({
      companyCode: requestedCompanyCode,
    });

    return res.status(200).json({
      currentElections: currentElectionsWithCandidates,
      pastElections: pastElectionsWithCandidates,
