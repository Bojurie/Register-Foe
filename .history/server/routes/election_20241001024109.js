...
router.get("/elections/:companyCode", verifyToken, async (req, res) => {
  ...
  try {
    // Fetch elections by company code and populate candidates information
    ...
    // Process each election to calculate candidate vote percentages
    ...
    return res.status(500).json({
      message: "An error occurred while fetching the elections data.",
      error: error.message,
    });
  }
});
