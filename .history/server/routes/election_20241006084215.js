...
router.post("/create-election", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Ensure all candidate IDs are valid MongoDB ObjectIds
    ...
    // Verify that all candidate IDs exist in the User collection
    ...
    // Create the Election document without the candidates
    const newElection = new Election({
      ...electionData,
      electionId: uuidv4(),
    });

    // Save the Election document
    await newElection.save({ session });

    // Create the Candidate documents and link them to the Election
    ...
    // Update the election with the candidates
    newElection.candidates = newCandidates.map((candidate) => ({
