...
router.post("/elections/:electionId/vote", verifyToken, async (req, res) => {
  ...
  const { electionId } = req.params; // MongoDB ObjectId
  const userId = req.user._id; // This is populated by verifyToken middleware

  console.log("User ID making the vote:", userId); // Log the user making the vote
  console.log("Received electionId:", electionId); // Log election ID
  console.log("Received candidateId:", candidateId); // Log candidate ID

  try {
    // Validate the existence of the election
    ...
    // Add the election to the user's list of voted elections
