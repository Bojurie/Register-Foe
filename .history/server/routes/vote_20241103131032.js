      ...
      .map((comment) => {
        ...
        const replies = comment.replies
          ? comment.replies.map((reply) => {
              ...
              return {
                id: reply._id || "No ID", // Fallback if ID is not present
                user: replyUser,
                text: reply.text,
                createdAt: new Date(reply.createdAt), // Use Date for accurate handling
        ...
        return {
          id: comment._id || "No ID", // Fallback if ID is not present
          user: commentUser,
          text: comment.text,
          createdAt: new Date(comment.createdAt), // Use Date for accurate handling
          replies: replies,
