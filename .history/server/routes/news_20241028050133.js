...
// Route to fetch news by company code with read status for current user
router.get("/byCompanyCode/:companyCode", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Process each news item, including image path and read status
    const newsWithImagesAndStatus = newsItems.map((item) => ({
      ...item,
      image: processImagePath(item.image),
      isNew: !(item.readBy || []).includes(userId),
    }));

    return res.status(200).json({
      news: newsWithImagesAndStatus,
      count: newsWithImagesAndStatus.length,
    });
  } catch (error) {
...
// Route to mark a news item as read
