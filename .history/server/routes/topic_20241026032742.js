...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Ensure the company code is present in the request params
      if (!requestedCompanyCode) {
        console.warn("Company code is missing in request parameters.");
        return res.status(400).json({ message: "Company code is required." });
      }

      // Check for access permissions based on user role and company association
      ...
      // Fetch only topics that are current or upcoming, sorted by start date in descending order
      ...
      if (!topics.length) {
        console.warn(
          `No current topics found for company code: ${requestedCompanyCode}`
        );
        return res.status(200).json({
          topics: [],
          message: "No current or upcoming topics found for this company.",
        });
      }
