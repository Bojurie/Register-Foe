...
router.get("/byCompanyCode/:companyCode", verifyToken, async (req, res) => {
  ...
  if (!requestedCompanyCode) {
    console.warn("[WARN] No company code provided.");
    return res.status(400).json({ error: "Company code is required." });
  }

  try {
    if (requestedCompanyCode !== userCompanyCode && role !== "Admin") {
      console.warn(`[WARN] Unauthorized access attempt by userId: ${userId}`);
    ...
    if (!newsItems.length) {
      console.log(
        `[INFO] No news found for company code: ${requestedCompanyCode}`
      );
    ...
    console.log(
      `[INFO] Processed ${newsWithReadStatus.length} news items with read status for userId: ${userId}`
    );

    return res.status(200).json({
      news: newsWithReadStatus,
      count: newsWithReadStatus.length,
    });
  } catch (error) {
...
router.patch("/markAsRead/:newsId", verifyToken, async (req, res) => {
  ...
  console.log(
    `[INFO] MarkAsRead route called for newsId: ${newsId} by userId: ${userId}`
  );
  ...
  try {
    ...
    // Only add userId if it’s not already in the readBy array
    if (!news.readBy.includes(userId)) {
      news.readBy.push(userId);
      await news.save();
      console.log(
        `[INFO] News item ${newsId} marked as read by userId: ${userId}`
      );
    } else {
      console.log(
        `[INFO] News item ${newsId} was already marked as read by userId: ${userId}`
      );
    }
