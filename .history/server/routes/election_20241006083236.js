  ...
  try {
    ...
    const newCandidates = await Promise.all(
      candidateDocs.map((user) =>
        new Candidate({
          user: user._id,
          election: null, // Will be set after the election is created
        }).save({ session })
      )
    );
    ...
    const newElection = new Election({
      ...electionData,
      electionId: uuidv4(),
      candidates: newCandidates.map((candidate) => candidate._id),
    });

    // Save the Election document
    await newElection.save({ session });

    // Update each Candidate to set the correct election reference
    await Promise.all(
