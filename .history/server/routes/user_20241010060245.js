...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Check if company exists with the given companyCode
      ...
      // Hash the password using bcrypt (handled in the schema pre-save middleware)
      ...
      const token = jwt.sign(
        { _id: user._id, isCompany: false }, // Assuming this is a regular user
        process.env.JWT_SECRET,
        { expiresIn: "1h" }
      );

      // Return success response with token (no password or sensitive data returned)
      res.status(201).json({
        user: {
          _id: user._id,
          email: user.email,
