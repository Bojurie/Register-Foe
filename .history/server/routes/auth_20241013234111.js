...
const bcrypt = require("bcrypt");
const Company = require("../models/Company"); // Make sure you require correct models
const User = require("../models/User");
const { generateToken } = require("../utils/jwt"); // Ensure this function exists to generate JWT tokens
...
router.post("/register", async (req, res) => {
  try {
    ...
    // Validate that password and necessary fields are present
    ...
    if (type === "company") {
      ...
      if (companyExists) {
        return res
          .status(409)
          .json({
            error: "Company with that name or username already exists.",
          });
      }
...
router.post("/login", async (req, res) => {
  ...
  try {
    // Fetch the user or company by username
