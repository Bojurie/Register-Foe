...
router.post("/users/:userId/like", verifyToken, async (req, res) => {
  ...
  try {
    // Ensure user can't like their own profile
    ...
    // Fetch user whose profile is being liked
    const user = await User.findById(userId);
    if (!user) {
      return res.status(404).json({ message: "User not found" });
    }

    // Increment profile likes count
    user.profileLikes = (user.profileLikes || 0) + 1;
    await user.save();

    return res.status(200).json({
      message: "Profile liked successfully",
      profileLikes: user.profileLikes, // Ensure this field is returned
    });
  } catch (error) {
