...
router.post("/register", async (req, res) => {
  try {
    console.log("Received registration data:", req.body); // Debug: Log incoming request data

    const { type, ...data } = req.body;

    if (!type) {
      console.error("No type provided in the registration request."); // Debug: Log missing type
      return res.status(400).json({ error: "Invalid registration type." });
    }

    if (type === "company") {
      console.log("Registering a company with data:", data); // Debug: Log company data

      if (!data.companyName || !data.username) {
        console.error("Missing required company registration data."); // Debug: Log missing fields
        return res
          .status(400)
      ...
      if (companyExists) {
        console.error("Company already exists with provided data."); // Debug: Log if company exists
      ...
      const newCompany = await Company.create({
        ...
        role: "Admin", // Set default role for companies
        isCompany: true, // Set isCompany to true for companies
      });

      console.log("Company registered successfully:", newCompany); // Debug: Log the new company data
      return res.status(201).json({
        message: "Company registered successfully.",
        companyId: newCompany._id,
      });
    } else if (type === "user") {
      console.log("Registering a user with data:", data); // Debug: Log user data

      if (!data.username || !data.email) {
        console.error("Missing required user registration data."); // Debug: Log missing fields
        return res
          .status(400)
      ...
      if (userExists) {
        console.error("User already exists with provided data."); // Debug: Log if user exists
        return res.status(409).json({
      ...
      const newUser = await User.create({
        ...
        isCompany: false, // Set isCompany to false for users
        age, // Calculate and assign age based on date of birth
      });

      const token = generateToken(newUser);
      console.log("User registered successfully:", newUser); // Debug: Log the new user data
      return res.status(201).json({
      ...
      console.error("Invalid registration type received:", type); // Debug: Log invalid type
      return res.status(400).json({ error: "Invalid registration type." });
    }
  } catch (error) {
    console.error("Error in registration:", error); // Debug: Log full error details
