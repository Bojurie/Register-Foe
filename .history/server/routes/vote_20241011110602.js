...
// Optimized dynamic endpoint for handling likes and dislikes on topics
router.post("/topics/:id/:action", verifyToken, async (req, res) => {
  const { id, action } = req.params;
  ...
  try {
    // Fetch the topic and make sure it exists
    const topic = await Topic.findById(id);
    if (!topic) {
      return res.status(404).json({ message: "Topic not found" });
    }

    const reactionsField = action === "like" ? "likes" : "dislikes";
    const oppositeReactionsField = action === "like" ? "dislikes" : "likes";

    // Remove user from the opposite reactions array if they had already reacted
    topic.reactions[oppositeReactionsField] = topic.reactions[oppositeReactionsField].filter(
      (user) => user.toString() !== userId
    );

    // Toggle the current reaction
    const userIndex = topic.reactions[reactionsField].findIndex((user) => user.toString() === userId);
    if (userIndex > -1) {
      // User already reacted, remove the reaction (toggle off)
      topic.reactions[reactionsField].splice(userIndex, 1);
    } else {
      // Add the user reaction
      topic.reactions[reactionsField].push(userId);
    }

    // Save the updated topic
    await topic.save();

    // Return updated likes and dislikes counts
    const likesCount = topic.reactions.likes.length;
    const dislikesCount = topic.reactions.dislikes.length;

    return res.status(200).json({
      message: `Successfully ${action}d the topic.`,
      likesCount,
      dislikesCount,
    });
  } catch (error) {
    console.error(`Error processing ${action} action for topic:`, error);
