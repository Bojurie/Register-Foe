...
//   console.log("Received request to /elections/:companyCode with params:", {
//     requestedCompanyCode,
//   });
//   console.log("User details from token:", { userCompanyCode, role });

//   if (!requestedCompanyCode) {
//     console.warn("No company code provided in the request.");
...
//     if (requestedCompanyCode !== userCompanyCode && role !== "Admin") {
//       console.warn(
//         "Access denied: User does not have permission to access this company's elections."
//       );
//       return res
//         .status(403)
...
//     const currentDate = new Date();
//     console.log(
//       "Fetching elections for company code:",
//       requestedCompanyCode,
//       "from date:",
//       currentDate
//     );
...
//       companyCode: requestedCompanyCode,
//     });
//     console.log("Total elections for company:", totalElections);

//     const currentElectionsCount = await Election.countDocuments({
//       companyCode: requestedCompanyCode,
//       endDate: { $gte: currentDate },
//     });
//     console.log("Current ongoing/upcoming elections:", currentElectionsCount);
...
//     console.log("Fetched elections data:", elections);

//     if (!elections.length) {
//       console.log("No upcoming elections found for this company.");
//       return res.status(200).json({
//         elections: [],
//         totalElections,
//         currentElections: currentElectionsCount,
...
router.get("/elections/:companyCode", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const elections = await Election.find({
      companyCode: requestedCompanyCode,
      endDate: { $gte: currentDate }, // Ensure fetching ongoing/upcoming elections
    })
      .populate({
        path: "candidates.candidateId",
        model: "User",
    ...
    // Process candidates to calculate vote percentages
    const electionsWithCandidates = elections.map((election) => {
      const totalVotes = election.candidates.reduce(
        (acc, candidate) => acc + (candidate.voteCount || 0),
        0
    ...
    console.log("Final response data:", {
      elections: electionsWithCandidates,
      totalElections,
      currentElections: electionsWithCandidates.length,
    });
    ...
    return res.status(500).json({
      error: "An error occurred while fetching the election data.",
    });
  }
});
