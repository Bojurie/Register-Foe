...
router.post("/save-elections", verifyToken, async (req, res) => {
  try {
    ...
    const newSavedElections = electionIds
      .filter((electionId) => !existingElectionIds.includes(electionId))
      .map((electionId) => ({
        electionRef: electionId,
        userRef,
      }));

    if (newSavedElections.length > 0) {
      ...
      console.log("All elections have already been saved by the user");
      return res.status(409).json({
        error: "All elections have already been saved by the user",
      });
    }
  } catch (error) {
