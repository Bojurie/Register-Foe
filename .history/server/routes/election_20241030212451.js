...
router.get("/elections/:companyCode", verifyToken, async (req, res) => {
  ...
  // Validate the requested company code
  if (!requestedCompanyCode) {
  ...
  try {
    ...
    // Calculate total elections count
    const totalElectionsCount = currentElections.length + pastElections.length;

    // Transform current elections for response
    ...
    return res.status(200).json({
      currentElections: transformElections(currentElections),
      pastElections: transformElections(pastElections),
