...
router.get("/byCompanyCode/:companyCode", verifyToken, async (req, res) => {
  ...
  if (!requestedCompanyCode) {
    console.warn("[WARN] No company code provided in the request.");
  ...
  try {
    ...
    if (newsItems.length > 0) {
      console.log(`[INFO] Retrieved ${newsItems.length} news items.`);
    } else {
      console.warn(
        "[WARN] No news items found for the requested company code."
      );
    }
