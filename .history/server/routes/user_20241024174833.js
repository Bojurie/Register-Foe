...
router.get(
  ...
  async (req, res) => {
    ...
    if (!requestedCompanyCode || requestedCompanyCode === "undefined") {
      console.warn("Company code is missing or invalid in request params.");
      return res.status(400).json({ error: "Valid company code is required" });
    }
    ...
    try {
      ...
      // Log the total number of users with the same company code only if valid
      console.log(
        `Total users for companyCode ${requestedCompanyCode}: ${users.length}`
      );

      // If users are found, return users with calculated virtual fields
      if (users.length > 0) {
      ...
      // If no users are found, return a message with 0 users
      return res.status(200).json({
        totalUsers: 0,
        users: [],
      ...
      // Log the error for debugging, but return a generic error message to the client
