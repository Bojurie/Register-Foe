...
router.get("/election/candidates/:id", verifyToken, async (req, res) => {
  try {
...
router.get("/election/:electionId", verifyToken, async (req, res) => {
  ...
  try {
    // Find the election by ID and ensure the company code matches for proper access control
    ...
    // Compute the total votes and percentages for each candidate
    ...
    return res.status(200).json({ election: electionWithCandidates });
  } catch (error) {
