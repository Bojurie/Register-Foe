...
router.post("/topics/:topicId/comments", verifyToken, async (req, res) => {
  ...
  // Validate that the comment text is present
  if (!text || typeof text !== "string") {
    return res
      .status(400)
      .json({ message: "Comment text is required and must be a string." });
  }
  ...
  try {
    ...
    // If the topic does not exist, return a 404
    if (!topic) {
      return res.status(404).json({ message: "Topic not found" });
    }

    // Create the new comment with the user's ID and the provided text
    const newComment = {
      user: req.user._id, // Ensure verifyToken adds req.user correctly
      text: text.trim(),
    };

    // Push the new comment into the topic's comments array
