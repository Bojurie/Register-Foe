...
// USER VOTE
router.post("/users/vote", verifyToken, async (req, res) => {
  const { userId, electionId } = req.body;

  if (!userId || !electionId) {
    return res
      .status(400)
      .json({ error: "userId and electionId are required" });
  }
  ...
  if (
    ...
    return res.status(400).json({ error: "Invalid userId or electionId" });
  }
  ...
  try {
    if (req.user.isCompany) {
      return res
        .status(403)
        .json({ error: "Companies are not allowed to vote in elections" });
    }
