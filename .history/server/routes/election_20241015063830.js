...
router.get("/elections/past/:companyCode", verifyToken, async (req, res) => {
  try {
    ...
    // Check if user has access to the requested company's elections
    ...
    // Fetch past elections depending on user's role
    if (isCompany || role === "Admin") {
      elections = await Election.find({
        companyCode: requestedCompanyCode,
        endDate: { $lt: currentDate },
      }).lean(); // Simple fetch without population for now
    } else {
      elections = await Election.find({
        companyCode: requestedCompanyCode,
        endDate: { $lt: currentDate },
        $or: [{ "candidates.user": userId }, { "candidates.voters": userId }],
      }).lean();
    }

    // If no elections are found, return an empty list with a message
    if (!elections.length) {
...
router.get("/elections/:companyCode", verifyToken, async (req, res) => {
  ...
  try {
    // Ensure the user and company share the same companyCode or the user is an Admin
    if (requestedCompanyCode !== userCompanyCode && role !== "Admin") {
      return res
        .status(403)
        .json({ error: "You do not have access to this company's elections." });
    ...
    // Fetch elections for the requested companyCode that are upcoming
    const elections = await Election.find({
      companyCode: requestedCompanyCode,
      endDate: { $gt: currentDate },
    })
    ...
    if (!elections.length) {
      // Return a response indicating no upcoming elections found
      return res.status(200).json({
        elections: [],
    ...
    // Calculate total votes and vote percentages for each candidate
