...
// Route for fetching likes or dislikes
router.get("/vote/topics/:id/:action", verifyToken, async (req, res) => {
  const { id, action } = req.params;

  // Validate the action parameter to ensure it's either 'likes' or 'dislikes'
  if (!["likes", "dislikes"].includes(action)) {
    return res.status(400).json({ message: "Invalid action. Use 'likes' or 'dislikes'." });
  }
  ...
  try {
    ...
    // Check that the reactions field exists and return the count and the reaction details
    const count = topic.reactions[action].length;

    return res.status(200).json({
      message: `Successfully fetched ${action} count.`,
      [`${action}Count`]: count,
      [`${action}`]: topic.reactions[action],  // List of users who liked/disliked
    });
  } catch (error) {
