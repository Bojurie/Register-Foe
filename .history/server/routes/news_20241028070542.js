...
router.get("/byCompanyCode/:companyCode", verifyToken, async (req, res) => {
  ...
  console.log(
    `[INFO] Fetching news for company code: ${companyCode} by userId: ${userId}`
  );
  ...
  try {
    ...
    console.log(
      `[INFO] Retrieved ${newsWithReadStatus.length} news items for company code: ${companyCode}`
    );
...
router.patch("/markAsRead/:newsId", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Only update if the user hasn't already marked it as read
