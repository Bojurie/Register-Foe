  ...
  async (req, res) => {
    ...
    try {
      ...
      if (commentId) {
        ...
        const newReply = { user: req.user._id, text: text.trim() };
        ...
        const newComment = { user: req.user._id, text: text.trim() };
      ...
      console.error("Error adding comment or reply:", error.message);
      return res
        .status(500)
        .json({ message: "Server error.", error: error.message });
    }
