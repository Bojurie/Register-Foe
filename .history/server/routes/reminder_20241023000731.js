...
router.get("/reminders/:userId", async (req, res) => {
  ...
  try {
    ...
    // Loop over reminders to filter and remove past reminders
    for (const reminder of reminders) {
      if (new Date(reminder.date) < currentDate) {
        await Reminder.findByIdAndDelete(reminder._id); // Remove past reminders
      } else {
        activeReminders.push(reminder); // Collect future reminders
      }
    }

    // Send only future reminders to the client
