...
// Get candidates of an electioIDn
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Find the election and populate candidates
      const election = await Election.findById(electionId).populate({
        path: "candidates",
        populate: {
          path: "user",
          model: "User",
          select: "firstName lastName age userProfileImage votesCount", // Select necessary fields
        },
      });
      ...
      // If no candidates exist for the election
