...
router.get("/elections/:companyCode?", verifyToken, async (req, res) => {
  ...
  try {
    // If a company code is provided, fetch elections specific to that company
    let electionsQuery = { dateEnd: { $gt: currentDate } }; // Default query for active elections

    if (companyCode) {
      electionsQuery.companyCode = companyCode;
    }
    ...
    // Process election data: calculate total votes and percentages
    const electionsWithCandidates = elections.map((election) => {
      const totalVotes = election.candidates.reduce(
        (acc, candidate) => acc + (candidate.voteCount || 0),
        0
