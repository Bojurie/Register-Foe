...
// Route to handle fetching likes for a topic
router.get("/vote/topics/:topicId/likes", verifyToken, async (req, res) => {
  const { topicId } = req.params;
  ...
  try {
    ...
    // Return the likes count and the users who liked the topic
    return res.status(200).json({
      likesCount: topic.reactions.likeCount,
      likes: topic.reactions.likes, // Array of users who liked
    });
  } catch (error) {
