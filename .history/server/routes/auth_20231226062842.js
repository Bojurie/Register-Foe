...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    const newUser = new User({ firstName, lastName, email, username, password: hashedPassword, companyCode });
    await newUser.save();
...
router.post("/login", async (req, res) => {
  ...
  try {
    // Determine the model (User or Company) based on the isCompany flag
    const Model = isCompany ? Company : User;
    const account = await Model.findOne({ username });

    // If no account is found, or the password doesn't match, return a generic error message
    if (!account) {
    ...
    const tokenPayload = { id: account._id, username, isCompany };
    const token = jwt.sign(tokenPayload, process.env.JWT_SECRET, {
      expiresIn: "1h",
    });

    // Respond with the token and relevant user information
    res.json({
      token,
      user: isCompany
        ? {
