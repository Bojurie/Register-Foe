...
router.post("/topics/:topicId/comments/comment", verifyToken, async (req, res) => {
  ...
  // Validate that the comment text is provided and is a string
  ...
  try {
    ...
    // If topic not found, send a 404 response
    if (!topic) {
      return res.status(404).json({ message: "Topic not found." });
    }
    ...
    const newComment = {
      user: req.user._id, // req.user._id comes from verifyToken middleware
      text: text.trim(),
    };

    // Add the new comment to the topic's comments array
    topic.comments.push(newComment);
    await topic.save(); // Save the updated topic with the new comment
