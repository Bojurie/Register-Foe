...
topicSchema.methods.likeComment = async function (commentId, userId) {
  const comment = this.comments.id(commentId); // Find the comment by its ID
  if (!comment) throw new Error("Comment not found");

  if (!comment.reactions.likes.some((id) => id.equals(userId))) {
...
topicSchema.methods.dislikeComment = async function (commentId, userId) {
  ...
  if (!comment.reactions.dislikes.some((id) => id.equals(userId))) {
