...
const userSchema = new mongoose.Schema(
  {
    ...
    hiredDate: {
      type: Date,
      default: Date.now, // Default to the registration date (createdAt)
    },
    votedElections: [
      {
...
userSchema.virtual("birthdayCountdown").get(function () {
  ...
  let nextBirthday = new Date(this.dob);
  nextBirthday.setFullYear(currentYear);

  if (now > nextBirthday) {
    nextBirthday.setFullYear(currentYear + 1); // Set to next year if birthday already passed this year
  }
...
userSchema.virtual("isNewHire").get(function () {
  ...
  oneMonthAgo.setMonth(oneMonthAgo.getMonth() - 1); // One month ago
...
userSchema.virtual("anniversaryCountdown").get(function () {
  const now = new Date();
  const currentYear = now.getFullYear();
  let anniversary = new Date(this.hiredDate);
  anniversary.setFullYear(currentYear);

  if (now > anniversary) {
    anniversary.setFullYear(currentYear + 1); // Set to next year if anniversary already passed this year
  }
